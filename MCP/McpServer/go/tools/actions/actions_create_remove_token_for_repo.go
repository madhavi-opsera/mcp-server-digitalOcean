...
func Actions_create_remove_token_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/runners/remove-token", cfg.BaseURL, owner, repo)
...
func CreateActions_create_remove_token_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_actions_runners_remove-token",
		mcp.WithDescription("Create a remove token for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_create_remove_token_for_repoHandler(cfg),
	}
}
