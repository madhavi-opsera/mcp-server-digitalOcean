...
func Actions_get_reviews_for_runHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/runs/%s/approvals", cfg.BaseURL, owner, repo, run_id)
		...
		var result []models.GeneratedType_Environment_approvals
...
func CreateActions_get_reviews_for_runTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_actions_runs_run_id_approvals",
		mcp.WithDescription("Get the review history for a workflow run"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_get_reviews_for_runHandler(cfg),
	}
}
