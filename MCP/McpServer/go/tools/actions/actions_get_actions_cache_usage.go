...
func Actions_get_actions_cache_usageHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/cache/usage", cfg.BaseURL, owner, repo)
		...
		var result models.GeneratedType_Actions_cache_usage_by_repository
...
func CreateActions_get_actions_cache_usageTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_actions_cache_usage",
		mcp.WithDescription("Get GitHub Actions cache usage for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_get_actions_cache_usageHandler(cfg),
	}
}
