...
func Actions_list_repo_variablesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateActions_list_repo_variablesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_actions_variables",
		mcp.WithDescription("List repository variables"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_list_repo_variablesHandler(cfg),
	}
}
