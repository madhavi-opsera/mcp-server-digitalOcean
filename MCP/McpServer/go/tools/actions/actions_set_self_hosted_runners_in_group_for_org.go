...
func Actions_set_self_hosted_runners_in_group_for_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/actions/runner-groups/%s/runners", cfg.BaseURL, org, runner_group_id)
...
func CreateActions_set_self_hosted_runners_in_group_for_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_orgs_org_actions_runner-groups_runner_group_id_runners_2",
		mcp.WithDescription("Set self-hosted runners in a group for an organization"),
		...
		mcp.WithArray("runners", mcp.Required(), mcp.Description("Input parameter: List of runner IDs to add to the runner group.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Actions_set_self_hosted_runners_in_group_for_orgHandler(cfg),
	}
}
