...
func Actions_remove_selected_repo_from_org_variableHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/actions/variables/%s/repositories/%s", cfg.BaseURL, org, name, repository_id)
...
func CreateActions_remove_selected_repo_from_org_variableTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_orgs_org_actions_variables_name_repositories_repository_id",
		mcp.WithDescription("Remove selected repository from an organization variable"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_remove_selected_repo_from_org_variableHandler(cfg),
	}
}
