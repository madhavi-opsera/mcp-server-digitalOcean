...
func Actions_delete_actions_cache_by_keyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var result models.GeneratedType_Actions_cache_list
...
func CreateActions_delete_actions_cache_by_keyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_actions_caches",
		mcp.WithDescription("Delete GitHub Actions caches for a repository (using a cache key)"),
		...
		mcp.WithString("key", mcp.Required(), mcp.Description("A key for identifying the cache.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_delete_actions_cache_by_keyHandler(cfg),
	}
}
