...
func Actions_list_selected_repositories_enabled_github_actions_organizationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateActions_list_selected_repositories_enabled_github_actions_organizationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_actions_permissions_repositories",
		mcp.WithDescription("List selected repositories enabled for GitHub Actions in an organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_list_selected_repositories_enabled_github_actions_organizationHandler(cfg),
	}
}
