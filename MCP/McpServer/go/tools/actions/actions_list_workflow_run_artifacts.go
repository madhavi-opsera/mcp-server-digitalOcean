...
func Actions_list_workflow_run_artifactsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/runs/%s/artifacts%s", cfg.BaseURL, owner, repo, run_id, queryString)
...
func CreateActions_list_workflow_run_artifactsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_actions_runs_run_id_artifacts",
		mcp.WithDescription("List workflow run artifacts"),
		...
		mcp.WithString("name", mcp.Description("The name field of an artifact. When specified, only artifacts with this name will be returned.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Actions_list_workflow_run_artifactsHandler(cfg),
	}
}
