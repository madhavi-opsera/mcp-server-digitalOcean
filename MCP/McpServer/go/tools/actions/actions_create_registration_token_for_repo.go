...
func Actions_create_registration_token_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/runners/registration-token", cfg.BaseURL, owner, repo)
		...
		var result models.GeneratedType_Authentication_token
...
func CreateActions_create_registration_token_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_actions_runners_registration-token",
		mcp.WithDescription("Create a registration token for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_create_registration_token_for_repoHandler(cfg),
	}
}
