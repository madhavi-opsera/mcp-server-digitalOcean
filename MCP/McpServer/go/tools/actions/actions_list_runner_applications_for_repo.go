...
func Actions_list_runner_applications_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/runners/downloads", cfg.BaseURL, owner, repo)
		...
		var result []models.GeneratedType_Runner_application
...
func CreateActions_list_runner_applications_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_actions_runners_downloads",
		mcp.WithDescription("List runner applications for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_list_runner_applications_for_repoHandler(cfg),
	}
}
