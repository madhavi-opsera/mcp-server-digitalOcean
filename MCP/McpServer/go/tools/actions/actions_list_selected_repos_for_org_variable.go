...
func Actions_list_selected_repos_for_org_variableHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateActions_list_selected_repos_for_org_variableTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_actions_variables_name_repositories",
		mcp.WithDescription("List selected repositories for an organization variable"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_list_selected_repos_for_org_variableHandler(cfg),
	}
}
