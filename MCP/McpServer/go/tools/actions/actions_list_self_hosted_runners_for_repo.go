...
func Actions_list_self_hosted_runners_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/runners%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateActions_list_self_hosted_runners_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_actions_runners",
		mcp.WithDescription("List self-hosted runners for a repository"),
		mcp.WithString("name", mcp.Description("The name of a self-hosted runner.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_list_self_hosted_runners_for_repoHandler(cfg),
	}
}
