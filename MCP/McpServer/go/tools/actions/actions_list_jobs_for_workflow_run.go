...
func Actions_list_jobs_for_workflow_runHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/runs/%s/jobs%s", cfg.BaseURL, owner, repo, run_id, queryString)
...
func CreateActions_list_jobs_for_workflow_runTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_actions_runs_run_id_jobs",
		mcp.WithDescription("List jobs for a workflow run"),
		...
		mcp.WithString("filter", mcp.Description("Filters jobs by their `completed_at` timestamp. `latest` returns jobs from the most recent execution of the workflow run. `all` returns all jobs for a workflow run, including from old executions of the workflow run.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_list_jobs_for_workflow_runHandler(cfg),
	}
}
