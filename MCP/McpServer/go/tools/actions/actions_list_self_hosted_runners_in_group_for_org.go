...
func Actions_list_self_hosted_runners_in_group_for_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/actions/runner-groups/%s/runners%s", cfg.BaseURL, org, runner_group_id, queryString)
...
func CreateActions_list_self_hosted_runners_in_group_for_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_actions_runner-groups_runner_group_id_runners",
		mcp.WithDescription("List self-hosted runners in a group for an organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_list_self_hosted_runners_in_group_for_orgHandler(cfg),
	}
}
