...
func Actions_get_actions_cache_usage_for_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/actions/cache/usage", cfg.BaseURL, org)
		...
		var result models.GeneratedType_Actions_cache_usage_org_enterprise
...
func CreateActions_get_actions_cache_usage_for_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_actions_cache_usage",
		mcp.WithDescription("Get GitHub Actions cache usage for an organization"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Actions_get_actions_cache_usage_for_orgHandler(cfg),
	}
}
