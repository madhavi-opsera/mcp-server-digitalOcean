...
func Actions_get_actions_cache_usage_by_repo_for_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/actions/cache/usage-by-repository%s", cfg.BaseURL, org, queryString)
...
func CreateActions_get_actions_cache_usage_by_repo_for_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_actions_cache_usage-by-repository",
		mcp.WithDescription("List repositories with GitHub Actions cache usage for an organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_get_actions_cache_usage_by_repo_for_orgHandler(cfg),
	}
}
