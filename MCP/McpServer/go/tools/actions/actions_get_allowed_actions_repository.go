...
func Actions_get_allowed_actions_repositoryHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateActions_get_allowed_actions_repositoryTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_actions_permissions_selected-actions",
		mcp.WithDescription("Get allowed actions and reusable workflows for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_get_allowed_actions_repositoryHandler(cfg),
	}
}
