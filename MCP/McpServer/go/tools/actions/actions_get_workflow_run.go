...
func Actions_get_workflow_runHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/runs/%s%s", cfg.BaseURL, owner, repo, run_id, queryString)
...
func CreateActions_get_workflow_runTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_actions_runs_run_id",
		mcp.WithDescription("Get a workflow run"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_get_workflow_runHandler(cfg),
	}
}
