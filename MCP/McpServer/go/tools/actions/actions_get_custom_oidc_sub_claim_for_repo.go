...
func Actions_get_custom_oidc_sub_claim_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var result models.GeneratedType_Oidc_custom_sub_repo
...
func CreateActions_get_custom_oidc_sub_claim_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_actions_oidc_customization_sub",
		mcp.WithDescription("Get the customization template for an OIDC subject claim for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_get_custom_oidc_sub_claim_for_repoHandler(cfg),
	}
}
