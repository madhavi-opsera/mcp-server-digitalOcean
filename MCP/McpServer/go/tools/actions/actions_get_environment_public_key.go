...
func Actions_get_environment_public_keyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/environments/%s/secrets/public-key", cfg.BaseURL, owner, repo, environment_name)
		...
		var result models.GeneratedType_Actions_public_key
...
func CreateActions_get_environment_public_keyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_environments_environment_name_secrets",
		mcp.WithDescription("Get an environment public key"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_get_environment_public_keyHandler(cfg),
	}
}
