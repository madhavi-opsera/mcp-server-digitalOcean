...
func Actions_get_workflow_run_usageHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/runs/%s/timing", cfg.BaseURL, owner, repo, run_id)
		...
		var result models.GeneratedType_Workflow_run_usage
...
func CreateActions_get_workflow_run_usageTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_actions_runs_run_id_timing",
		mcp.WithDescription("Get workflow run usage"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_get_workflow_run_usageHandler(cfg),
	}
}
