...
func Teams_create_discussion_comment_in_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/teams/%s/discussions/%s/comments", cfg.BaseURL, org, team_slug, discussion_number)
...
func CreateTeams_create_discussion_comment_in_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_orgs_org_teams_team_slug_discussions_discussion_number_comments",
		mcp.WithDescription("Create a discussion comment"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_create_discussion_comment_in_orgHandler(cfg),
	}
}
