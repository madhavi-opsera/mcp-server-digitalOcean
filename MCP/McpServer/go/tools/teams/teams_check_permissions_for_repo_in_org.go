...
func Teams_check_permissions_for_repo_in_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/teams/%s/repos/%s/%s", cfg.BaseURL, org, team_slug, owner, repo)
		...
		var result models.GeneratedType_Team_repository
...
func CreateTeams_check_permissions_for_repo_in_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_teams_team_slug_repos_owner_repo",
		mcp.WithDescription("Check team permissions for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_check_permissions_for_repo_in_orgHandler(cfg),
	}
}
