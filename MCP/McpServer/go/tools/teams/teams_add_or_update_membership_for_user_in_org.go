...
func Teams_add_or_update_membership_for_user_in_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/teams/%s/memberships/%s", cfg.BaseURL, org, team_slug, username)
		...
		var result models.GeneratedType_Team_membership
...
func CreateTeams_add_or_update_membership_for_user_in_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_orgs_org_teams_team_slug_memberships_username",
		mcp.WithDescription("Add or update team membership for a user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_add_or_update_membership_for_user_in_orgHandler(cfg),
	}
}
