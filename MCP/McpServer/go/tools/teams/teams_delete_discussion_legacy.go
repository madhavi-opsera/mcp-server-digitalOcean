...
func Teams_delete_discussion_legacyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateTeams_delete_discussion_legacyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_teams_team_id_discussions_discussion_number",
		mcp.WithDescription("Delete a discussion (Legacy)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_delete_discussion_legacyHandler(cfg),
	}
}
