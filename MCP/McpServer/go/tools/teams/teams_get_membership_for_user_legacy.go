...
func Teams_get_membership_for_user_legacyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateTeams_get_membership_for_user_legacyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_teams_team_id_memberships_username",
		mcp.WithDescription("Get team membership for a user (Legacy)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_get_membership_for_user_legacyHandler(cfg),
	}
}
