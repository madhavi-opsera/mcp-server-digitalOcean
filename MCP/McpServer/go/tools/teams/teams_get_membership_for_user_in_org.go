...
func Teams_get_membership_for_user_in_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateTeams_get_membership_for_user_in_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_teams_team_slug_memberships_username",
		mcp.WithDescription("Get team membership for a user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_get_membership_for_user_in_orgHandler(cfg),
	}
}
