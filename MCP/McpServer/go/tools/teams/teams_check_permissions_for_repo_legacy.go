...
func Teams_check_permissions_for_repo_legacyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateTeams_check_permissions_for_repo_legacyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_teams_team_id_repos_owner_repo",
		mcp.WithDescription("Check team permissions for a repository (Legacy)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_check_permissions_for_repo_legacyHandler(cfg),
	}
}
