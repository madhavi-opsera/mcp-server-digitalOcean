...
func Teams_delete_legacyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateTeams_delete_legacyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_teams_team_id",
		mcp.WithDescription("Delete a team (Legacy)"),
		mcp.WithNumber("team_id", mcp.Required(), mcp.Description("The unique identifier of the team.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Teams_delete_legacyHandler(cfg),
	}
}
