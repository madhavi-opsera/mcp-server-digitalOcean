...
func Teams_list_discussions_legacyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateTeams_list_discussions_legacyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_teams_team_id_discussions",
		mcp.WithDescription("List discussions (Legacy)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_list_discussions_legacyHandler(cfg),
	}
}
