...
func Teams_list_pending_invitations_in_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/teams/%s/invitations%s", cfg.BaseURL, org, team_slug, queryString)
...
func CreateTeams_list_pending_invitations_in_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_teams_team_slug_invitations",
		mcp.WithDescription("List pending team invitations"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_list_pending_invitations_in_orgHandler(cfg),
	}
}
