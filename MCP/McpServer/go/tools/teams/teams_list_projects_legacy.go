...
func Teams_list_projects_legacyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/teams/%s/projects%s", cfg.BaseURL, team_id, queryString)
...
func CreateTeams_list_projects_legacyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_teams_team_id_projects",
		mcp.WithDescription("List team projects (Legacy)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_list_projects_legacyHandler(cfg),
	}
}
