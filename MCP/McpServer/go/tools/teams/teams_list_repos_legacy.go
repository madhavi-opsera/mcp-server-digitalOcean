...
func Teams_list_repos_legacyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/teams/%s/repos%s", cfg.BaseURL, team_id, queryString)
...
func CreateTeams_list_repos_legacyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_teams_team_id_repos",
		mcp.WithDescription("List team repositories (Legacy)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_list_repos_legacyHandler(cfg),
	}
}
