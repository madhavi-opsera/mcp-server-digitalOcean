...
func Teams_check_permissions_for_project_legacyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/teams/%s/projects/%s", cfg.BaseURL, team_id, project_id)
		...
		var result models.GeneratedType_Team_project
...
func CreateTeams_check_permissions_for_project_legacyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_teams_team_id_projects_project",
		mcp.WithDescription("Check team permissions for a project (Legacy)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_check_permissions_for_project_legacyHandler(cfg),
	}
}
