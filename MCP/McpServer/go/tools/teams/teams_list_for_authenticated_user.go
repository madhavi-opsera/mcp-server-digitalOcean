...
func Teams_list_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/teams%s", cfg.BaseURL, queryString)
...
func CreateTeams_list_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_user_teams",
		mcp.WithDescription("List teams for the authenticated user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_list_for_authenticated_userHandler(cfg),
	}
}
