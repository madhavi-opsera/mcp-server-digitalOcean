...
func Teams_list_projects_in_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/teams/%s/projects%s", cfg.BaseURL, org, team_slug, queryString)
...
func CreateTeams_list_projects_in_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_teams_team_slug_projects",
		mcp.WithDescription("List team projects"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_list_projects_in_orgHandler(cfg),
	}
}
