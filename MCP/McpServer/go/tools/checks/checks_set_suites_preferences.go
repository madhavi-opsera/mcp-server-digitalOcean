...
func Checks_set_suites_preferencesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/check-suites/preferences", cfg.BaseURL, owner, repo)
		...
		var result models.GeneratedType_Check_suite_preference
...
func CreateChecks_set_suites_preferencesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_repos_owner_repo_check-suites_preferences",
		mcp.WithDescription("Update repository preferences for check suites"),
		...
		mcp.WithArray("auto_trigger_checks", mcp.Description("Input parameter: Enables or disables automatic creation of CheckSuite events upon pushes to the repository. Enabled by default.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Checks_set_suites_preferencesHandler(cfg),
	}
}
