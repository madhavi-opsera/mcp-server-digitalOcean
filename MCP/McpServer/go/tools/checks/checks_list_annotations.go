...
func Checks_list_annotationsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/check-runs/%s/annotations%s", cfg.BaseURL, owner, repo, check_run_id, queryString)
		...
		var result []models.GeneratedType_Check_annotation
...
func CreateChecks_list_annotationsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_check-runs_check_run_id_annotations",
		mcp.WithDescription("List check run annotations"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Checks_list_annotationsHandler(cfg),
	}
}
