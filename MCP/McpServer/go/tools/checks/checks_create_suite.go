...
func Checks_create_suiteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/check-suites", cfg.BaseURL, owner, repo)
		...
		var result models.GeneratedType_Check_suite
...
func CreateChecks_create_suiteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_check-suites",
		mcp.WithDescription("Create a check suite"),
		...
		mcp.WithString("head_sha", mcp.Required(), mcp.Description("Input parameter: The sha of the head commit.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Checks_create_suiteHandler(cfg),
	}
}
