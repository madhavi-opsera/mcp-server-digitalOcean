...
func Checks_get_suiteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/check-suites/%s", cfg.BaseURL, owner, repo, check_suite_id)
...
func CreateChecks_get_suiteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_check-suites_check_suite_id",
		mcp.WithDescription("Get a check suite"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Checks_get_suiteHandler(cfg),
	}
}
