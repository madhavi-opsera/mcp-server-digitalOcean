...
func Checks_list_for_refHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["check_name"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("check_name=%v", val))
		}
		...
		url := fmt.Sprintf("%s/repos/%s/%s/commits/%s/check-runs%s", cfg.BaseURL, owner, repo, ref, queryString)
...
func CreateChecks_list_for_refTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_commits_ref_check-runs",
		mcp.WithDescription("List check runs for a Git reference"),
		...
		mcp.WithString("ref", mcp.Required(), mcp.Description("The commit reference. Can be a commit SHA, branch name (`heads/BRANCH_NAME`), or tag name (`tags/TAG_NAME`). For more information, see \"[Git References](https://git-scm.com/book/en/v2/Git-Internals-Git-References)\" in the Git documentation.")),
		mcp.WithString("check_name", mcp.Description("Returns check runs with the specified `name`.")),
		mcp.WithString("status", mcp.Description("Returns check runs with the specified `status`.")),
		mcp.WithString("filter", mcp.Description("Filters check runs by their `completed_at` timestamp. `latest` returns the most recent check runs.")),
		...
		mcp.WithNumber("app_id", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Checks_list_for_refHandler(cfg),
	}
}
