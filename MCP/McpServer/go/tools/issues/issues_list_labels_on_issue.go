...
func Issues_list_labels_on_issueHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateIssues_list_labels_on_issueTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_issues_issue_number_labels",
		mcp.WithDescription("List labels for an issue"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Issues_list_labels_on_issueHandler(cfg),
	}
}
