...
func Issues_list_events_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/issues/events%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateIssues_list_events_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_issues_events",
		mcp.WithDescription("List issue events for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Issues_list_events_for_repoHandler(cfg),
	}
}
