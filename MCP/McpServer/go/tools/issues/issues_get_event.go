...
func Issues_get_eventHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		event_idVal, ok := args["event_id"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: event_id"), nil
		}
		event_id, ok := event_idVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: event_id"), nil
		}
		url := fmt.Sprintf("%s/repos/%s/%s/issues/events/%s", cfg.BaseURL, owner, repo, event_id)
		...
		var result models.GeneratedType_Issue_event
...
func CreateIssues_get_eventTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_issues_events_event_id",
		mcp.WithDescription("Get an issue event"),
		...
		mcp.WithNumber("event_id", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Issues_get_eventHandler(cfg),
	}
}
