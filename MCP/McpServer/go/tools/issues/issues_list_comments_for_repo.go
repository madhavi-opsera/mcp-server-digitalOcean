...
func Issues_list_comments_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/issues/comments%s", cfg.BaseURL, owner, repo, queryString)
		...
		var result []models.GeneratedType_Issue_comment
...
func CreateIssues_list_comments_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_issues_comments",
		mcp.WithDescription("List issue comments for a repository"),
		...
		mcp.WithString("direction", mcp.Description("Either `asc` or `desc`. Ignored without the `sort` parameter.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Issues_list_comments_for_repoHandler(cfg),
	}
}
