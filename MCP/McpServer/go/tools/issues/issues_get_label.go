...
func Issues_get_labelHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateIssues_get_labelTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_labels_name",
		mcp.WithDescription("Get a label"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Issues_get_labelHandler(cfg),
	}
}
