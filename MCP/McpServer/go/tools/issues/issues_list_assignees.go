...
func Issues_list_assigneesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/assignees%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateIssues_list_assigneesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_assignees",
		mcp.WithDescription("List assignees"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Issues_list_assigneesHandler(cfg),
	}
}
