...
func Issues_list_events_for_timelineHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/issues/%s/timeline%s", cfg.BaseURL, owner, repo, issue_number, queryString)
		...
		var result []models.GeneratedType_Timeline_issue_events
...
func CreateIssues_list_events_for_timelineTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_issues_issue_number_timeline",
		mcp.WithDescription("List timeline events for an issue"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Issues_list_events_for_timelineHandler(cfg),
	}
}
