...
func Private_registries_list_org_private_registriesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePrivate_registries_list_org_private_registriesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_private-registries",
		mcp.WithDescription("List private registries for an organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Private_registries_list_org_private_registriesHandler(cfg),
	}
}
