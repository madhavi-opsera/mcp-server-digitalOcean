...
func Migrations_get_large_filesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/import/large_files", cfg.BaseURL, owner, repo)
		...
		var result []models.GeneratedType_Porter_large_file
...
func CreateMigrations_get_large_filesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_import_large_files",
		mcp.WithDescription("Get large files"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Migrations_get_large_filesHandler(cfg),
	}
}
