...
func Migrations_list_for_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateMigrations_list_for_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_migrations",
		mcp.WithDescription("List organization migrations"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Migrations_list_for_orgHandler(cfg),
	}
}
