...
func Migrations_cancel_importHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateMigrations_cancel_importTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_import",
		mcp.WithDescription("Cancel an import"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Migrations_cancel_importHandler(cfg),
	}
}
