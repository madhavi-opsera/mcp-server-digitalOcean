...
func Migrations_get_status_for_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/migrations/%s%s", cfg.BaseURL, org, migration_id, queryString)
...
func CreateMigrations_get_status_for_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_migrations_migration_id",
		mcp.WithDescription("Get an organization migration status"),
		...
		mcp.WithArray("exclude", mcp.Description("Exclude attributes from the API response to improve performance")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Migrations_get_status_for_orgHandler(cfg),
	}
}
