...
func Migrations_get_commit_authorsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/import/authors%s", cfg.BaseURL, owner, repo, queryString)
		...
		var result []models.GeneratedType_Porter_author
...
func CreateMigrations_get_commit_authorsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_import_authors",
		mcp.WithDescription("Get commit authors"),
		...
		mcp.WithNumber("since", mcp.Description("A user ID. Only return users with an ID greater than this ID.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Migrations_get_commit_authorsHandler(cfg),
	}
}
