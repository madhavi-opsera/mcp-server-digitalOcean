...
func Migrations_list_repos_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/migrations/%s/repositories%s", cfg.BaseURL, migration_id, queryString)
...
func CreateMigrations_list_repos_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_user_migrations_migration_id_repositories",
		mcp.WithDescription("List repositories for a user migration"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Migrations_list_repos_for_authenticated_userHandler(cfg),
	}
}
