...
func Migrations_list_repos_for_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/migrations/%s/repositories%s", cfg.BaseURL, org, migration_id, queryString)
...
func CreateMigrations_list_repos_for_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_migrations_migration_id_repositories",
		mcp.WithDescription("List repositories in an organization migration"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Migrations_list_repos_for_orgHandler(cfg),
	}
}
