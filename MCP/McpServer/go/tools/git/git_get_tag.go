...
func Git_get_tagHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		tag_shaVal, ok := args["tag_sha"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: tag_sha"), nil
		}
		tag_sha, ok := tag_shaVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: tag_sha"), nil
		}
		url := fmt.Sprintf("%s/repos/%s/%s/git/tags/%s", cfg.BaseURL, owner, repo, tag_sha)
		...
		var result models.GeneratedType_Git_tag
...
func CreateGit_get_tagTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_git_tags_tag_sha",
		...
		mcp.WithString("tag_sha", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Git_get_tagHandler(cfg),
	}
}
