...
func Codespaces_list_devcontainers_in_repository_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/codespaces/devcontainers%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateCodespaces_list_devcontainers_in_repository_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_codespaces_devcontainers",
		mcp.WithDescription("List devcontainer configurations in a repository for the authenticated user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_list_devcontainers_in_repository_for_authenticated_userHandler(cfg),
	}
}
