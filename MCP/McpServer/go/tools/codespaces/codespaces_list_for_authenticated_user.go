...
func Codespaces_list_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/codespaces%s", cfg.BaseURL, queryString)
...
func CreateCodespaces_list_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_user_codespaces",
		mcp.WithDescription("List codespaces for the authenticated user"),
		...
		mcp.WithNumber("repository_id", mcp.Description("ID of the Repository to filter on")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_list_for_authenticated_userHandler(cfg),
	}
}
