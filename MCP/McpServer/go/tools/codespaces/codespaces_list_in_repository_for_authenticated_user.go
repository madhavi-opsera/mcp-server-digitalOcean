...
func Codespaces_list_in_repository_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCodespaces_list_in_repository_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_codespaces",
		mcp.WithDescription("List codespaces in a repository for the authenticated user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_list_in_repository_for_authenticated_userHandler(cfg),
	}
}
