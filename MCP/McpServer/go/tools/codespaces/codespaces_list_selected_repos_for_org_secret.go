...
func Codespaces_list_selected_repos_for_org_secretHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/codespaces/secrets/%s/repositories%s", cfg.BaseURL, org, secret_name, queryString)
...
func CreateCodespaces_list_selected_repos_for_org_secretTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_codespaces_secrets_secret_name_repositories",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_list_selected_repos_for_org_secretHandler(cfg),
	}
}
