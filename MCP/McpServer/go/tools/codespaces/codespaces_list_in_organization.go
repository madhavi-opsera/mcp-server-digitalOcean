...
func Codespaces_list_in_organizationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/codespaces%s", cfg.BaseURL, org, queryString)
...
func CreateCodespaces_list_in_organizationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_codespaces",
		mcp.WithDescription("List codespaces for the organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_list_in_organizationHandler(cfg),
	}
}
