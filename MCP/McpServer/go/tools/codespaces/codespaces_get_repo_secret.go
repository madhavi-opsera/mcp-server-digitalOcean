...
func Codespaces_get_repo_secretHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/codespaces/secrets/%s", cfg.BaseURL, owner, repo, secret_name)
		...
		var result models.GeneratedType_Repo_codespaces_secret
...
func CreateCodespaces_get_repo_secretTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_codespaces_secrets_secret_name",
		mcp.WithDescription("Get a repository secret"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_get_repo_secretHandler(cfg),
	}
}
