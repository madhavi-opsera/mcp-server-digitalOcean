...
func Codespaces_get_repo_public_keyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/codespaces/secrets/public-key", cfg.BaseURL, owner, repo)
		...
		var result models.GeneratedType_Codespaces_public_key
...
func CreateCodespaces_get_repo_public_keyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_codespaces_secrets_public-key",
		mcp.WithDescription("Get a repository public key"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_get_repo_public_keyHandler(cfg),
	}
}
