...
func Codespaces_get_org_public_keyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/codespaces/secrets/public-key", cfg.BaseURL, org)
...
func CreateCodespaces_get_org_public_keyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_codespaces_secrets_public-key",
		mcp.WithDescription("Get an organization public key"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_get_org_public_keyHandler(cfg),
	}
}
