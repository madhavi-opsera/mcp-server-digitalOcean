...
func Codespaces_remove_repository_for_secret_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCodespaces_remove_repository_for_secret_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_user_codespaces_secrets_secret_name_repositories_repository_id",
		mcp.WithDescription("Remove a selected repository from a user secret"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_remove_repository_for_secret_for_authenticated_userHandler(cfg),
	}
}
