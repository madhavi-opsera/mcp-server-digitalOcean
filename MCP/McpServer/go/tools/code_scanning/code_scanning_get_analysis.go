...
func Code_scanning_get_analysisHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/code-scanning/analyses/%s", cfg.BaseURL, owner, repo, analysis_id)
		...
		var result models.GeneratedType_Code_scanning_analysis
...
func CreateCode_scanning_get_analysisTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_code-scanning_analyses_analysis_id",
		mcp.WithDescription("Get a code scanning analysis for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Code_scanning_get_analysisHandler(cfg),
	}
}
