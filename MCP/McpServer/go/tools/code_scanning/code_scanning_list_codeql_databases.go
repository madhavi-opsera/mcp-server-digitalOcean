...
func Code_scanning_list_codeql_databasesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/code-scanning/codeql/databases", cfg.BaseURL, owner, repo)
...
func CreateCode_scanning_list_codeql_databasesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_code-scanning_codeql_databases",
		mcp.WithDescription("List CodeQL databases for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Code_scanning_list_codeql_databasesHandler(cfg),
	}
}
