...
func Code_scanning_get_sarifHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		sarif_idVal, ok := args["sarif_id"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: sarif_id"), nil
		}
		sarif_id, ok := sarif_idVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: sarif_id"), nil
		}
		url := fmt.Sprintf("%s/repos/%s/%s/code-scanning/sarifs/%s", cfg.BaseURL, owner, repo, sarif_id)
		...
		var result models.GeneratedType_Code_scanning_sarifs_status
...
func CreateCode_scanning_get_sarifTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_code-scanning_sarifs_sarif_id",
		mcp.WithDescription("Get information about a SARIF upload"),
		...
		mcp.WithString("sarif_id", mcp.Required(), mcp.Description("The SARIF ID obtained after uploading.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Code_scanning_get_sarifHandler(cfg),
	}
}
