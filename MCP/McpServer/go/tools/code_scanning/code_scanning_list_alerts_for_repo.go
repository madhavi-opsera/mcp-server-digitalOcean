...
func Code_scanning_list_alerts_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/code-scanning/alerts%s", cfg.BaseURL, owner, repo, queryString)
		...
		var result []models.GeneratedType_Code_scanning_alert_items
...
func CreateCode_scanning_list_alerts_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_code-scanning_alerts",
		mcp.WithDescription("List code scanning alerts for a repository"),
		...
		mcp.WithString("ref", mcp.Description("The Git reference for the results you want to list. The `ref` for a branch can be formatted either as `refs/heads/<branch name>` or simply `<branch name>`. To reference a pull request use `refs/pull/<number>/merge`.")),
		...
		mcp.WithString("state", mcp.Description("If specified, only code scanning alerts with this state will be returned.")),
		mcp.WithString("severity", mcp.Description("If specified, only code scanning alerts with this severity will be returned.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Code_scanning_list_alerts_for_repoHandler(cfg),
	}
}
