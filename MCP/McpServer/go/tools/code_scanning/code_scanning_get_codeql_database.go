...
func Code_scanning_get_codeql_databaseHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/code-scanning/codeql/databases/%s", cfg.BaseURL, owner, repo, language)
		...
		var result models.GeneratedType_Code_scanning_codeql_database
...
func CreateCode_scanning_get_codeql_databaseTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_code-scanning_codeql_databases_language",
		mcp.WithDescription("Get a CodeQL database for a repository"),
		...
		mcp.WithString("language", mcp.Required(), mcp.Description("The language of the CodeQL database.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Code_scanning_get_codeql_databaseHandler(cfg),
	}
}
