...
func Secret_scanning_list_alerts_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/secret-scanning/alerts%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateSecret_scanning_list_alerts_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_secret-scanning_alerts",
		mcp.WithDescription("List secret scanning alerts for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Secret_scanning_list_alerts_for_repoHandler(cfg),
	}
}
