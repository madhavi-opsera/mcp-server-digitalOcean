...
func Secret_scanning_list_locations_for_alertHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/secret-scanning/alerts/%s/locations%s", cfg.BaseURL, owner, repo, alert_number, queryString)
		...
		var result []models.GeneratedType_Secret_scanning_location
...
func CreateSecret_scanning_list_locations_for_alertTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_secret-scanning_alerts_alert_number",
		mcp.WithDescription("List locations for a secret scanning alert"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Secret_scanning_list_locations_for_alertHandler(cfg),
	}
}
