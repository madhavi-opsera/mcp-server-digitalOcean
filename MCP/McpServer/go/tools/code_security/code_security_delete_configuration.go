...
func Code_security_delete_configurationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCode_security_delete_configurationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_orgs_org_code-security_configurations_configuration_id",
		mcp.WithDescription("Delete a code security configuration"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Code_security_delete_configurationHandler(cfg),
	}
}
