...
func Reactions_list_for_issue_commentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/issues/comments/%s/reactions%s", cfg.BaseURL, owner, repo, comment_id, queryString)
...
func CreateReactions_list_for_issue_commentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_issues_comments_comment_id_reactions",
		mcp.WithDescription("List reactions for an issue comment"),
		...
		mcp.WithString("content", mcp.Description("Returns a single [reaction type](https://docs.github.com/rest/reactions/reactions#about-reactions). Omit this parameter to list all reactions to an issue comment.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Reactions_list_for_issue_commentHandler(cfg),
	}
}
