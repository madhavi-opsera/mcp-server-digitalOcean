...
func Reactions_list_for_issueHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/issues/%s/reactions%s", cfg.BaseURL, owner, repo, issue_number, queryString)
...
func CreateReactions_list_for_issueTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_issues_issue_number_reactions",
		mcp.WithDescription("List reactions for an issue"),
		...
		mcp.WithString("content", mcp.Description("Returns a single [reaction type](https://docs.github.com/rest/reactions/reactions#about-reactions). Omit this parameter to list all reactions to an issue.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Reactions_list_for_issueHandler(cfg),
	}
}
