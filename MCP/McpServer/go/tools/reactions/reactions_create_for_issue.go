...
func Reactions_create_for_issueHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/issues/%s/reactions", cfg.BaseURL, owner, repo, issue_number)
...
func CreateReactions_create_for_issueTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_issues_issue_number_reactions",
		mcp.WithDescription("Create reaction for an issue"),
		...
		mcp.WithString("content", mcp.Required(), mcp.Description("Input parameter: The [reaction type](https://docs.github.com/rest/reactions/reactions#about-reactions) to add to the issue.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Reactions_create_for_issueHandler(cfg),
	}
}
