...
func Reactions_list_for_team_discussion_in_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateReactions_list_for_team_discussion_in_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_teams_team_slug_discussions_discussion_number_reactions",
		mcp.WithDescription("List reactions for a team discussion"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Reactions_list_for_team_discussion_in_orgHandler(cfg),
	}
}
