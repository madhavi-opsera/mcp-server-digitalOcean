...
func Codes_of_conduct_get_conduct_codeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/codes_of_conduct/%s", cfg.BaseURL, key)
...
func CreateCodes_of_conduct_get_conduct_codeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_codes_of_conduct_key",
		mcp.WithDescription("Get a code of conduct"),
		mcp.WithString("key", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Codes_of_conduct_get_conduct_codeHandler(cfg),
	}
}
