...
func Pulls_list_filesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/pulls/%s/files%s", cfg.BaseURL, owner, repo, pull_number, queryString)
		...
		var result []models.GeneratedType_Diff_entry
...
func CreatePulls_list_filesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_pulls_pull_number_files",
		mcp.WithDescription("List pull requests files"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Pulls_list_filesHandler(cfg),
	}
}
