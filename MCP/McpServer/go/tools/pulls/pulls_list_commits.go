...
func Pulls_list_commitsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/pulls/%s/commits%s", cfg.BaseURL, owner, repo, pull_number, queryString)
...
func CreatePulls_list_commitsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_pulls_pull_number_commits",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Pulls_list_commitsHandler(cfg),
	}
}
