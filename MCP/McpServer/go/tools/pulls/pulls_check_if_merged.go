...
func Pulls_check_if_mergedHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePulls_check_if_mergedTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_pulls_pull_number_merge",
		mcp.WithDescription("Check if a pull request has been merged"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Pulls_check_if_mergedHandler(cfg),
	}
}
