...
func Pulls_list_review_commentsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePulls_list_review_commentsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_pulls_pull_number_comments",
		mcp.WithDescription("List review comments on a pull request"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Pulls_list_review_commentsHandler(cfg),
	}
}
