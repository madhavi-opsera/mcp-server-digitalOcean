...
func Pulls_list_review_comments_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/pulls/comments%s", cfg.BaseURL, owner, repo, queryString)
...
func CreatePulls_list_review_comments_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_pulls_comments",
		mcp.WithDescription("List review comments in a repository"),
		...
		mcp.WithString("direction", mcp.Description("The direction to sort results. Ignored without `sort` parameter.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Pulls_list_review_comments_for_repoHandler(cfg),
	}
}
