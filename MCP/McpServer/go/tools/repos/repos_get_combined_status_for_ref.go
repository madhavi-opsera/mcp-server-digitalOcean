...
func Repos_get_combined_status_for_refHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/commits/%s/status%s", cfg.BaseURL, owner, repo, ref, queryString)
		...
		var result models.GeneratedType_Combined_commit_status
...
func CreateRepos_get_combined_status_for_refTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_commits_ref_status",
		mcp.WithDescription("Get the combined status for a specific reference"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_combined_status_for_refHandler(cfg),
	}
}
