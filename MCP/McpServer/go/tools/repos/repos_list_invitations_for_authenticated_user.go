...
func Repos_list_invitations_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/repository_invitations%s", cfg.BaseURL, queryString)
...
func CreateRepos_list_invitations_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_user_repository_invitations",
		mcp.WithDescription("List repository invitations for the authenticated user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_invitations_for_authenticated_userHandler(cfg),
	}
}
