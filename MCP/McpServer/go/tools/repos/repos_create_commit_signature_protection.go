...
func Repos_create_commit_signature_protectionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/branches/%s/protection/required_signatures", cfg.BaseURL, owner, repo, branch)
		...
		var result models.GeneratedType_Protected_branch_admin_enforced
...
func CreateRepos_create_commit_signature_protectionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_branches_branch_protection_required",
		mcp.WithDescription("Create commit signature protection"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_create_commit_signature_protectionHandler(cfg),
	}
}
