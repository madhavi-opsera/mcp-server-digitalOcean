...
func Repos_get_webhookHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_get_webhookTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_hooks_hook_id",
		mcp.WithDescription("Get a repository webhook"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_webhookHandler(cfg),
	}
}
