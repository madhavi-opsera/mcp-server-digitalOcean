...
func Repos_list_forksHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_list_forksTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_forks",
		mcp.WithDescription("List forks"),
		...
		mcp.WithString("sort", mcp.Description("The sort order. `stargazers` will sort by star count.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_forksHandler(cfg),
	}
}
