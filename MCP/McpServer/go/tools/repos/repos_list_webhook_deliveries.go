...
func Repos_list_webhook_deliveriesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/hooks/%s/deliveries%s", cfg.BaseURL, owner, repo, hook_id, queryString)
		...
		var result []models.GeneratedType_Hook_delivery_item
...
func CreateRepos_list_webhook_deliveriesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_hooks_hook_id_deliveries",
		mcp.WithDescription("List deliveries for a repository webhook"),
		...
		mcp.WithString("cursor", mcp.Description("Used for pagination: the starting delivery from which the page of deliveries is fetched. Refer to the `link` header for the next and previous page cursors.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_webhook_deliveriesHandler(cfg),
	}
}
