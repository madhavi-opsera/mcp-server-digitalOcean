...
func Repos_get_commit_activity_statsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/stats/commit_activity", cfg.BaseURL, owner, repo)
		...
		var result []models.GeneratedType_Commit_activity
...
func CreateRepos_get_commit_activity_statsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_stats_commit_activity",
		mcp.WithDescription("Get the last year of commit activity"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_commit_activity_statsHandler(cfg),
	}
}
