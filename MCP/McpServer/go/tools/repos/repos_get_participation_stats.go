...
func Repos_get_participation_statsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/stats/participation", cfg.BaseURL, owner, repo)
		...
		var result models.GeneratedType_Participation_stats
...
func CreateRepos_get_participation_statsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_stats_participation",
		mcp.WithDescription("Get the weekly commit count"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_participation_statsHandler(cfg),
	}
}
