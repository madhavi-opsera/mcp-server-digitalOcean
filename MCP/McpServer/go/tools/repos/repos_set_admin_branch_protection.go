...
func Repos_set_admin_branch_protectionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/branches/%s/protection/enforce_admins", cfg.BaseURL, owner, repo, branch)
...
func CreateRepos_set_admin_branch_protectionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_branches_branch_protection_enforce",
		mcp.WithDescription("Set admin branch protection"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_set_admin_branch_protectionHandler(cfg),
	}
}
