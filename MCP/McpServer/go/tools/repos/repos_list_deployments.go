...
func Repos_list_deploymentsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["task"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("task=%v", val))
		}
...
func CreateRepos_list_deploymentsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_deployments",
		...
		mcp.WithString("sha", mcp.Description("The SHA recorded at creation time.")),
		mcp.WithString("ref", mcp.Description("The name of the ref. This can be a branch, tag, or SHA.")),
		mcp.WithString("task", mcp.Description("The name of the task for the deployment (e.g., `deploy` or `deploy:migrations`).")),
		mcp.WithString("environment", mcp.Description("The name of the environment that was deployed to (e.g., `staging` or `production`).")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_deploymentsHandler(cfg),
	}
}
