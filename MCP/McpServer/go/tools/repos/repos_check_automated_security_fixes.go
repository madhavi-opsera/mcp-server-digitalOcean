...
func Repos_check_automated_security_fixesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/automated-security-fixes", cfg.BaseURL, owner, repo)
		...
		var result models.GeneratedType_Check_automated_security_fixes
...
func CreateRepos_check_automated_security_fixesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_automated-security-fixes",
		mcp.WithDescription("Check if Dependabot security updates are enabled for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_check_automated_security_fixesHandler(cfg),
	}
}
