...
func Repos_get_pages_health_checkHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/pages/health", cfg.BaseURL, owner, repo)
		...
		var result models.GeneratedType_Pages_health_check
...
func CreateRepos_get_pages_health_checkTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_pages_health",
		mcp.WithDescription("Get a DNS health check for GitHub Pages"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_pages_health_checkHandler(cfg),
	}
}
