...
func Repos_get_status_checks_protectionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_get_status_checks_protectionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_branches_branch_protection_required_status_checks",
		mcp.WithDescription("Get status checks protection"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_status_checks_protectionHandler(cfg),
	}
}
