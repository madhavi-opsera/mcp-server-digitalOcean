...
func Repos_request_pages_buildHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/pages/builds", cfg.BaseURL, owner, repo)
		...
		var result models.GeneratedType_Page_build_status
...
func CreateRepos_request_pages_buildTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_pages_builds",
		mcp.WithDescription("Request a GitHub Pages build"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_request_pages_buildHandler(cfg),
	}
}
