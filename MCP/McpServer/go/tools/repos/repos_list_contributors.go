...
func Repos_list_contributorsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["anon"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("anon=%v", val))
		}
		...
		url := fmt.Sprintf("%s/repos/%s/%s/contributors%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateRepos_list_contributorsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_contributors",
		mcp.WithDescription("List repository contributors"),
		...
		mcp.WithString("anon", mcp.Description("Set to `1` or `true` to include anonymous contributors in results.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_contributorsHandler(cfg),
	}
}
