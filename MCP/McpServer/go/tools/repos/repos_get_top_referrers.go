...
func Repos_get_top_referrersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/traffic/popular/referrers", cfg.BaseURL, owner, repo)
		...
		var result []models.GeneratedType_Referrer_traffic
...
func CreateRepos_get_top_referrersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_traffic_popular_referrers",
		mcp.WithDescription("Get top referral sources"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_top_referrersHandler(cfg),
	}
}
