...
func Repos_get_webhook_config_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_get_webhook_config_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_hooks_hook_id_config",
		mcp.WithDescription("Get a webhook configuration for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_webhook_config_for_repoHandler(cfg),
	}
}
