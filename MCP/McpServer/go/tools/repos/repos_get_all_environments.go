...
func Repos_get_all_environmentsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/environments%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateRepos_get_all_environmentsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_environments",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_all_environmentsHandler(cfg),
	}
}
