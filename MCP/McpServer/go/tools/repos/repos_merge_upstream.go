...
func Repos_merge_upstreamHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/merge-upstream", cfg.BaseURL, owner, repo)
		...
		var result models.GeneratedType_Merged_upstream
...
func CreateRepos_merge_upstreamTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_merge-upstream",
		mcp.WithDescription("Sync a fork branch with the upstream repository"),
		...
		mcp.WithString("branch", mcp.Required(), mcp.Description("Input parameter: The name of the branch which should be updated to match upstream.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_merge_upstreamHandler(cfg),
	}
}
