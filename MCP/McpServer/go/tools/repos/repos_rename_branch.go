...
func Repos_rename_branchHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/branches/%s/rename", cfg.BaseURL, owner, repo, branch)
		...
		var result models.GeneratedType_Branch_with_protection
...
func CreateRepos_rename_branchTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_branches_branch_rename",
		mcp.WithDescription("Rename a branch"),
		...
		mcp.WithString("new_name", mcp.Required(), mcp.Description("Input parameter: The new name of the branch.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_rename_branchHandler(cfg),
	}
}
