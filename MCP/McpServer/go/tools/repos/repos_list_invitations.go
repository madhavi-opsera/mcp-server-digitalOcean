...
func Repos_list_invitationsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/invitations%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateRepos_list_invitationsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_invitations",
		mcp.WithDescription("List repository invitations"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_invitationsHandler(cfg),
	}
}
