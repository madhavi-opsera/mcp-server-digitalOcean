...
func Repos_get_all_topicsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_get_all_topicsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_topics",
		mcp.WithDescription("Get all repository topics"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_all_topicsHandler(cfg),
	}
}
