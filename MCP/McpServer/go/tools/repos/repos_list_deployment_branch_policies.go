...
func Repos_list_deployment_branch_policiesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_list_deployment_branch_policiesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_environments_environment_name_deployment-branch-policies",
		mcp.WithDescription("List deployment branch policies"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_deployment_branch_policiesHandler(cfg),
	}
}
