...
func Repos_list_branches_for_head_commitHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/commits/%s/branches-where-head", cfg.BaseURL, owner, repo, commit_sha)
		...
		var result []models.GeneratedType_Branch_short
...
func CreateRepos_list_branches_for_head_commitTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_commits_commit_sha_branches-where-head",
		mcp.WithDescription("List branches for HEAD commit"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_branches_for_head_commitHandler(cfg),
	}
}
