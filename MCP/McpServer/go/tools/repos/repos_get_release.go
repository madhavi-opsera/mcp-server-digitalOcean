...
func Repos_get_releaseHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_get_releaseTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_releases_release_id",
		mcp.WithDescription("Get a release"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_releaseHandler(cfg),
	}
}
