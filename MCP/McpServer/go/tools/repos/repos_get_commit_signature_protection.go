...
func Repos_get_commit_signature_protectionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_get_commit_signature_protectionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_branches_branch_protection_required",
		mcp.WithDescription("Get commit signature protection"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_commit_signature_protectionHandler(cfg),
	}
}
