...
func Repos_get_branch_rulesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/rules/branches/%s%s", cfg.BaseURL, owner, repo, branch, queryString)
		...
		var result []models.GeneratedType_Repository_rule_detailed
...
func CreateRepos_get_branch_rulesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_rules_branches_branch",
		mcp.WithDescription("Get rules for a branch"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_branch_rulesHandler(cfg),
	}
}
