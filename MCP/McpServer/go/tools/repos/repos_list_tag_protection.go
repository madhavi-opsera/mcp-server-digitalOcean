...
func Repos_list_tag_protectionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_list_tag_protectionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_tags_protection",
		mcp.WithDescription("Closing down - List tag protection states for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_tag_protectionHandler(cfg),
	}
}
