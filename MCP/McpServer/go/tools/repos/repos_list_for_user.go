...
func Repos_list_for_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/repos%s", cfg.BaseURL, username, queryString)
...
func CreateRepos_list_for_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_username_repos",
		mcp.WithDescription("List repositories for a user"),
		mcp.WithString("username", mcp.Required(), mcp.Description("The handle for the GitHub user account.")),
		mcp.WithString("type", mcp.Description("Limit results to repositories of the specified type.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_for_userHandler(cfg),
	}
}
