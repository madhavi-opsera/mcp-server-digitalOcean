...
func Repos_get_community_profile_metricsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/community/profile", cfg.BaseURL, owner, repo)
		...
		var result models.GeneratedType_Community_profile
...
func CreateRepos_get_community_profile_metricsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_community_profile",
		mcp.WithDescription("Get community profile metrics"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_community_profile_metricsHandler(cfg),
	}
}
