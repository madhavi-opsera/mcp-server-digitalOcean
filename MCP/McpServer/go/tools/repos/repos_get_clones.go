...
func Repos_get_clonesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["per"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("per=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/repos/%s/%s/traffic/clones%s", cfg.BaseURL, owner, repo, queryString)
		...
		var result models.GeneratedType_Clone_traffic
...
func CreateRepos_get_clonesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_traffic_clones",
		mcp.WithDescription("Get repository clones"),
		...
		mcp.WithString("per", mcp.Description("The time frame to display results for.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_clonesHandler(cfg),
	}
}
