...
func Repos_delete_deploymentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/deployments/%s", cfg.BaseURL, owner, repo, deployment_id)
...
func CreateRepos_delete_deploymentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_deployments_deployment_id",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_delete_deploymentHandler(cfg),
	}
}
