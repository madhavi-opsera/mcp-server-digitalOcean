...
func Repos_list_pages_buildsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/pages/builds%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateRepos_list_pages_buildsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_pages_builds",
		mcp.WithDescription("List GitHub Pages builds"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_pages_buildsHandler(cfg),
	}
}
