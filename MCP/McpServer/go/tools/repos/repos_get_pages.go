...
func Repos_get_pagesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_get_pagesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_pages",
		mcp.WithDescription("Get a GitHub Pages site"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_pagesHandler(cfg),
	}
}
