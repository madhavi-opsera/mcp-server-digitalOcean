...
func Repos_list_branchesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["protected"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("protected=%v", val))
		}
		...
		url := fmt.Sprintf("%s/repos/%s/%s/branches%s", cfg.BaseURL, owner, repo, queryString)
		...
		var result []models.GeneratedType_Short_branch
...
func CreateRepos_list_branchesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_branches",
		...
		mcp.WithBoolean("protected", mcp.Description("Setting to `true` returns only branches protected by branch protections or rulesets. When set to `false`, only unprotected branches are returned. Omitting this parameter returns all branches.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_branchesHandler(cfg),
	}
}
