...
func Repos_codeowners_errorsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/codeowners/errors%s", cfg.BaseURL, owner, repo, queryString)
		...
		var result models.GeneratedType_Codeowners_errors
...
func CreateRepos_codeowners_errorsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_codeowners_errors",
		mcp.WithDescription("List CODEOWNERS errors"),
		...
		mcp.WithString("ref", mcp.Description("A branch, tag or commit name used to determine which version of the CODEOWNERS file to use. Default: the repository's default branch (e.g. `main`)")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_codeowners_errorsHandler(cfg),
	}
}
