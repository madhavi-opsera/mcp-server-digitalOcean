...
func Repos_get_contributors_statsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/stats/contributors", cfg.BaseURL, owner, repo)
		...
		var result []models.GeneratedType_Contributor_activity
...
func CreateRepos_get_contributors_statsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_stats_contributors",
		mcp.WithDescription("Get all contributor commit activity"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_contributors_statsHandler(cfg),
	}
}
