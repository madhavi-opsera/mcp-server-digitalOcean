...
func Repos_get_top_pathsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/traffic/popular/paths", cfg.BaseURL, owner, repo)
		...
		var result []models.GeneratedType_Content_traffic
...
func CreateRepos_get_top_pathsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_traffic_popular_paths",
		mcp.WithDescription("Get top referral paths"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_top_pathsHandler(cfg),
	}
}
