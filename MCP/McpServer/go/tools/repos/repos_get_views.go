...
func Repos_get_viewsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/traffic/views%s", cfg.BaseURL, owner, repo, queryString)
		...
		var result models.GeneratedType_View_traffic
...
func CreateRepos_get_viewsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_traffic_views",
		mcp.WithDescription("Get page views"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_viewsHandler(cfg),
	}
}
