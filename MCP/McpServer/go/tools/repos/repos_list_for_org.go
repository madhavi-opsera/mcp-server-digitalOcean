...
func Repos_list_for_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_list_for_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_repos",
		mcp.WithDescription("List organization repositories"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
		mcp.WithString("type", mcp.Description("Specifies the types of repositories you want returned.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_for_orgHandler(cfg),
	}
}
