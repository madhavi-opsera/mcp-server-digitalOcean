...
func Repos_get_latest_pages_buildHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/pages/builds/latest", cfg.BaseURL, owner, repo)
		...
		var result models.GeneratedType_Page_build
...
func CreateRepos_get_latest_pages_buildTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_pages_builds_latest",
		mcp.WithDescription("Get latest Pages build"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_latest_pages_buildHandler(cfg),
	}
}
