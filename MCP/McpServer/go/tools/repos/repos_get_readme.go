...
func Repos_get_readmeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/readme%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateRepos_get_readmeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_readme",
		mcp.WithDescription("Get a repository README"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_readmeHandler(cfg),
	}
}
