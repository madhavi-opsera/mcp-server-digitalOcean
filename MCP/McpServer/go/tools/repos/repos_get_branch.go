...
func Repos_get_branchHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/branches/%s", cfg.BaseURL, owner, repo, branch)
...
func CreateRepos_get_branchTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_branches_branch",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_branchHandler(cfg),
	}
}
