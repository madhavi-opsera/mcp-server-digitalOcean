...
func Repos_list_commit_statuses_for_refHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/commits/%s/statuses%s", cfg.BaseURL, owner, repo, ref, queryString)
...
func CreateRepos_list_commit_statuses_for_refTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_commits_ref_statuses",
		mcp.WithDescription("List commit statuses for a reference"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_commit_statuses_for_refHandler(cfg),
	}
}
