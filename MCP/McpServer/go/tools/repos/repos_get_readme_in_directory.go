...
func Repos_get_readme_in_directoryHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		dirVal, ok := args["dir"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: dir"), nil
		}
		dir, ok := dirVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: dir"), nil
		}
		...
		url := fmt.Sprintf("%s/repos/%s/%s/readme/%s%s", cfg.BaseURL, owner, repo, dir, queryString)
		...
		var result models.GeneratedType_Content_file
...
func CreateRepos_get_readme_in_directoryTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_readme_dir",
		mcp.WithDescription("Get a repository README for a directory"),
		...
		mcp.WithString("dir", mcp.Required(), mcp.Description("The alternate path to look for a README file")),
		mcp.WithString("ref", mcp.Description("The name of the commit/branch/tag. Default: the repository’s default branch.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_readme_in_directoryHandler(cfg),
	}
}
