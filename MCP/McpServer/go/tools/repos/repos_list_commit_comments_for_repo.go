...
func Repos_list_commit_comments_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/comments%s", cfg.BaseURL, owner, repo, queryString)
		...
		var result []models.GeneratedType_Commit_comment
...
func CreateRepos_list_commit_comments_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_comments",
		mcp.WithDescription("List commit comments for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_commit_comments_for_repoHandler(cfg),
	}
}
