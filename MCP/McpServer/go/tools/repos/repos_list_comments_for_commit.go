...
func Repos_list_comments_for_commitHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_list_comments_for_commitTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_commits_commit_sha_comments",
		mcp.WithDescription("List commit comments"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_comments_for_commitHandler(cfg),
	}
}
