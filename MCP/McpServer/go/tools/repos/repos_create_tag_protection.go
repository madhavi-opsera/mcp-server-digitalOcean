...
func Repos_create_tag_protectionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/tags/protection", cfg.BaseURL, owner, repo)
		...
		var result models.GeneratedType_Tag_protection
...
func CreateRepos_create_tag_protectionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_tags_protection",
		mcp.WithDescription("Closing down - Create a tag protection state for a repository"),
		...
		mcp.WithString("pattern", mcp.Required(), mcp.Description("Input parameter: An optional glob pattern to match against when enforcing tag protection.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_create_tag_protectionHandler(cfg),
	}
}
