...
func Packages_get_all_package_versions_for_package_owned_by_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/packages/%s/%s/versions%s", cfg.BaseURL, package_type, package_name, org, queryString)
...
func CreatePackages_get_all_package_versions_for_package_owned_by_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_packages_package_type_package_name_versions",
		mcp.WithDescription("List package versions for a package owned by an organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Packages_get_all_package_versions_for_package_owned_by_orgHandler(cfg),
	}
}
