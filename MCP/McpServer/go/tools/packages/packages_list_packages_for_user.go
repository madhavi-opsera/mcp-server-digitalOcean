...
func Packages_list_packages_for_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/packages%s", cfg.BaseURL, username, queryString)
...
func CreatePackages_list_packages_for_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_username_packages",
		mcp.WithDescription("List packages for a user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Packages_list_packages_for_userHandler(cfg),
	}
}
