...
func Packages_get_all_package_versions_for_package_owned_by_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/packages/%s/%s/versions%s", cfg.BaseURL, package_type, package_name, queryString)
...
func CreatePackages_get_all_package_versions_for_package_owned_by_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_user_packages_package_type_package_name_versions",
		mcp.WithDescription("List package versions for a package owned by the authenticated user"),
		...
		mcp.WithString("state", mcp.Description("The state of the package, either active or deleted.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Packages_get_all_package_versions_for_package_owned_by_authenticated_userHandler(cfg),
	}
}
