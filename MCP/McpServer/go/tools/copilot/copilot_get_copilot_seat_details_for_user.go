...
func Copilot_get_copilot_seat_details_for_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/members/%s/copilot", cfg.BaseURL, org, username)
		...
		var result models.GeneratedType_Copilot_seat_details
...
func CreateCopilot_get_copilot_seat_details_for_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_members_username_copilot",
		mcp.WithDescription("Get Copilot seat assignment details for a user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Copilot_get_copilot_seat_details_for_userHandler(cfg),
	}
}
