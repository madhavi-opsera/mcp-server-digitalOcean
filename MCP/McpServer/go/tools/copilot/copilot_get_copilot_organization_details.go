...
func Copilot_get_copilot_organization_detailsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/copilot/billing", cfg.BaseURL, org)
		...
		var result models.GeneratedType_Copilot_organization_details
...
func CreateCopilot_get_copilot_organization_detailsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_copilot_billing",
		mcp.WithDescription("Get Copilot seat information and settings for an organization"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Copilot_get_copilot_organization_detailsHandler(cfg),
	}
}
