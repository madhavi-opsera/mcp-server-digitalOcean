...
func Dependency_graph_export_sbomHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/dependency-graph/sbom", cfg.BaseURL, owner, repo)
		...
		var result models.GeneratedType_Dependency_graph_spdx_sbom
...
func CreateDependency_graph_export_sbomTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_dependency-graph_sbom",
		mcp.WithDescription("Export a software bill of materials (SBOM) for a repository."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Dependency_graph_export_sbomHandler(cfg),
	}
}
