...
func Gitignore_get_templateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/gitignore/templates/%s", cfg.BaseURL, name)
		...
		var result models.GeneratedType_Gitignore_template
...
func CreateGitignore_get_templateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_gitignore_templates_name",
		mcp.WithDescription("Get a gitignore template"),
		mcp.WithString("name", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Gitignore_get_templateHandler(cfg),
	}
}
