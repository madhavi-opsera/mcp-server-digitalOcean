...
func Users_list_ssh_signing_keys_for_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/ssh_signing_keys%s", cfg.BaseURL, username, queryString)
...
func CreateUsers_list_ssh_signing_keys_for_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_username_ssh_signing_keys",
		mcp.WithDescription("List SSH signing keys for a user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Users_list_ssh_signing_keys_for_userHandler(cfg),
	}
}
