...
func Users_list_following_for_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/following%s", cfg.BaseURL, username, queryString)
...
func CreateUsers_list_following_for_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_username_following",
		mcp.WithDescription("List the people a user follows"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Users_list_following_for_userHandler(cfg),
	}
}
