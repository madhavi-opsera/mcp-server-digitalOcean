...
func Users_list_public_keys_for_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/keys%s", cfg.BaseURL, username, queryString)
		...
		var result []models.GeneratedType_Key_simple
...
func CreateUsers_list_public_keys_for_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_username_keys",
		mcp.WithDescription("List public keys for a user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Users_list_public_keys_for_userHandler(cfg),
	}
}
