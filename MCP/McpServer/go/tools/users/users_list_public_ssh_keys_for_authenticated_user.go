...
func Users_list_public_ssh_keys_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateUsers_list_public_ssh_keys_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_user_keys",
		mcp.WithDescription("List public SSH keys for the authenticated user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Users_list_public_ssh_keys_for_authenticated_userHandler(cfg),
	}
}
