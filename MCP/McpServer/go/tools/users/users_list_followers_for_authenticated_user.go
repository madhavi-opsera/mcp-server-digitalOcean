...
func Users_list_followers_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/followers%s", cfg.BaseURL, queryString)
...
func CreateUsers_list_followers_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_user_followers",
		mcp.WithDescription("List followers of the authenticated user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Users_list_followers_for_authenticated_userHandler(cfg),
	}
}
