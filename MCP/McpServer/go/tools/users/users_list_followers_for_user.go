...
func Users_list_followers_for_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/followers%s", cfg.BaseURL, username, queryString)
...
func CreateUsers_list_followers_for_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_username_followers",
		mcp.WithDescription("List followers of a user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Users_list_followers_for_userHandler(cfg),
	}
}
