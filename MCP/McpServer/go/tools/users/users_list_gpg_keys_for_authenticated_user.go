...
func Users_list_gpg_keys_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateUsers_list_gpg_keys_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_user_gpg_keys",
		mcp.WithDescription("List GPG keys for the authenticated user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Users_list_gpg_keys_for_authenticated_userHandler(cfg),
	}
}
