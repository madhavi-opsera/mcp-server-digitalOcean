...
func Users_list_public_emails_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/public_emails%s", cfg.BaseURL, queryString)
...
func CreateUsers_list_public_emails_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_user_public_emails",
		mcp.WithDescription("List public email addresses for the authenticated user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Users_list_public_emails_for_authenticated_userHandler(cfg),
	}
}
