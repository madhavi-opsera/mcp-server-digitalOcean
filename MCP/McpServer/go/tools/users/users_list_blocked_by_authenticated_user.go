...
func Users_list_blocked_by_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/blocks%s", cfg.BaseURL, queryString)
...
func CreateUsers_list_blocked_by_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_user_blocks",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Users_list_blocked_by_authenticated_userHandler(cfg),
	}
}
