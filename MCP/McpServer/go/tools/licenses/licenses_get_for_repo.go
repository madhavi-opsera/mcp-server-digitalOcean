...
func Licenses_get_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/license%s", cfg.BaseURL, owner, repo, queryString)
		...
		var result models.GeneratedType_License_content
...
func CreateLicenses_get_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_license",
		mcp.WithDescription("Get the license for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Licenses_get_for_repoHandler(cfg),
	}
}
