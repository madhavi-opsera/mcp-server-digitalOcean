...
func Licenses_get_all_commonly_usedHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["featured"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("featured=%v", val))
		}
		...
		url := fmt.Sprintf("%s/licenses%s", cfg.BaseURL, queryString)
		...
		var result []models.GeneratedType_License_simple
...
func CreateLicenses_get_all_commonly_usedTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_licenses",
		mcp.WithDescription("Get all commonly used licenses"),
		mcp.WithBoolean("featured", mcp.Description("")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Licenses_get_all_commonly_usedHandler(cfg),
	}
}
