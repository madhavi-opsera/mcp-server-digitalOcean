...
func Activity_list_public_eventsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateActivity_list_public_eventsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_events",
		mcp.WithDescription("List public events"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Activity_list_public_eventsHandler(cfg),
	}
}
