...
func Activity_list_watchers_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/subscribers%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateActivity_list_watchers_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_subscribers",
		mcp.WithDescription("List watchers"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Activity_list_watchers_for_repoHandler(cfg),
	}
}
