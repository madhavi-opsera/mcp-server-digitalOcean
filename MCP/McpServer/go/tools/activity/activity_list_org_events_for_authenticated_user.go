...
func Activity_list_org_events_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/events/orgs/%s%s", cfg.BaseURL, username, org, queryString)
...
func CreateActivity_list_org_events_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_username_events_orgs_org",
		mcp.WithDescription("List organization events for the authenticated user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Activity_list_org_events_for_authenticated_userHandler(cfg),
	}
}
