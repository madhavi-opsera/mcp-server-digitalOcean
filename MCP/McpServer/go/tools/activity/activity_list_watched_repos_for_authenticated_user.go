...
func Activity_list_watched_repos_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/subscriptions%s", cfg.BaseURL, queryString)
...
func CreateActivity_list_watched_repos_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_user_subscriptions",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Activity_list_watched_repos_for_authenticated_userHandler(cfg),
	}
}
