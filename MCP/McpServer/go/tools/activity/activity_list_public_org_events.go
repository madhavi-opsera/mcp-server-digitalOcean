...
func Activity_list_public_org_eventsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/events%s", cfg.BaseURL, org, queryString)
...
func CreateActivity_list_public_org_eventsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_events",
		mcp.WithDescription("List public organization events"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Activity_list_public_org_eventsHandler(cfg),
	}
}
