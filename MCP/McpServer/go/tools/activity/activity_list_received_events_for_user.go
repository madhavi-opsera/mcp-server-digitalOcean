...
func Activity_list_received_events_for_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/received_events%s", cfg.BaseURL, username, queryString)
...
func CreateActivity_list_received_events_for_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_username_received_events",
		mcp.WithDescription("List events received by the authenticated user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Activity_list_received_events_for_userHandler(cfg),
	}
}
