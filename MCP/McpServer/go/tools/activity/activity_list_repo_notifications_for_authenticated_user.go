...
func Activity_list_repo_notifications_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/notifications%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateActivity_list_repo_notifications_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_notifications",
		mcp.WithDescription("List repository notifications for the authenticated user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Activity_list_repo_notifications_for_authenticated_userHandler(cfg),
	}
}
