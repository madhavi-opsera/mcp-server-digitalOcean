...
func Activity_list_repos_watched_by_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/subscriptions%s", cfg.BaseURL, username, queryString)
...
func CreateActivity_list_repos_watched_by_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_username_subscriptions",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Activity_list_repos_watched_by_userHandler(cfg),
	}
}
