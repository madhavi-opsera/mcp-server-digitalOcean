...
func Interactions_remove_restrictions_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateInteractions_remove_restrictions_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_interaction-limits",
		mcp.WithDescription("Remove interaction restrictions for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Interactions_remove_restrictions_for_repoHandler(cfg),
	}
}
