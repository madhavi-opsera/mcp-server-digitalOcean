...
func Search_commitsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/search/commits%s", cfg.BaseURL, queryString)
...
func CreateSearch_commitsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_search_commits",
		mcp.WithDescription("Search commits"),
		...
		mcp.WithString("sort", mcp.Description("Sorts the results of your query by `author-date` or `committer-date`. Default: [best match](https://docs.github.com/rest/search/search#ranking-search-results)")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Search_commitsHandler(cfg),
	}
}
