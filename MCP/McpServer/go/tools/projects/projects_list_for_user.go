...
func Projects_list_for_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/projects%s", cfg.BaseURL, username, queryString)
...
func CreateProjects_list_for_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_username_projects",
		mcp.WithDescription("List user projects"),
		mcp.WithString("username", mcp.Required(), mcp.Description("The handle for the GitHub user account.")),
		mcp.WithString("state", mcp.Description("Indicates the state of the projects to return.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Projects_list_for_userHandler(cfg),
	}
}
