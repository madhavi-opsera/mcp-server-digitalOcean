...
func Projects_list_for_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/projects%s", cfg.BaseURL, org, queryString)
...
func CreateProjects_list_for_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_projects",
		mcp.WithDescription("List organization projects"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Projects_list_for_orgHandler(cfg),
	}
}
