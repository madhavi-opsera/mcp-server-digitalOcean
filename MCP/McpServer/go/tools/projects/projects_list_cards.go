...
func Projects_list_cardsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["archived_state"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("archived_state=%v", val))
		}
		...
		url := fmt.Sprintf("%s/projects/columns/%s/cards%s", cfg.BaseURL, column_id, queryString)
		...
		var result []models.GeneratedType_Project_card
...
func CreateProjects_list_cardsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_projects_columns_column_id_cards",
		mcp.WithDescription("List project cards"),
		mcp.WithNumber("column_id", mcp.Required(), mcp.Description("The unique identifier of the column.")),
		mcp.WithString("archived_state", mcp.Description("Filters the project cards that are returned by the card's state.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Projects_list_cardsHandler(cfg),
	}
}
