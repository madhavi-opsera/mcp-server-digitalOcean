...
func Projects_create_columnHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/projects/%s/columns", cfg.BaseURL, project_id)
		...
		var result models.GeneratedType_Project_column
...
func CreateProjects_create_columnTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_projects_project_id_columns",
		mcp.WithDescription("Create a project column"),
		mcp.WithNumber("project_id", mcp.Required(), mcp.Description("The unique identifier of the project.")),
		mcp.WithString("name", mcp.Required(), mcp.Description("Input parameter: Name of the project column")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Projects_create_columnHandler(cfg),
	}
}
