...
func Projects_list_collaboratorsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/projects/%s/collaborators%s", cfg.BaseURL, project_id, queryString)
		...
		var result []models.GeneratedType_Simple_user
...
func CreateProjects_list_collaboratorsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_projects_project_id_collaborators",
		mcp.WithDescription("List project collaborators"),
		mcp.WithNumber("project_id", mcp.Required(), mcp.Description("The unique identifier of the project.")),
		mcp.WithString("affiliation", mcp.Description("Filters the collaborators by their affiliation. `outside` means outside collaborators of a project that are not a member of the project's organization. `direct` means collaborators with permissions to a project, regardless of organization membership status. `all` means all collaborators the authenticated user can see.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Projects_list_collaboratorsHandler(cfg),
	}
}
