...
func Projects_get_permission_for_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/projects/%s/collaborators/%s/permission", cfg.BaseURL, project_id, username)
		...
		var result models.GeneratedType_Project_collaborator_permission
...
func CreateProjects_get_permission_for_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_projects_project_id_collaborators_username_permission",
		mcp.WithDescription("Get project permission for a user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Projects_get_permission_for_userHandler(cfg),
	}
}
