...
func Apps_list_accounts_for_plan_stubbedHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		args, ok := request.Params.Arguments.(map[string]any)
		if !ok {
			return mcp.NewToolResultError("Invalid arguments object"), nil
		}
		plan_idVal, ok := args["plan_id"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: plan_id"), nil
		}
		plan_id, ok := plan_idVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: plan_id"), nil
		}
		...
		url := fmt.Sprintf("%s/marketplace_listing/stubbed/plans/%s/accounts%s", cfg.BaseURL, plan_id, queryString)
		...
		var result []models.GeneratedType_Marketplace_purchase
...
func CreateApps_list_accounts_for_plan_stubbedTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_marketplace_listing_stubbed_plans_plan_id_accounts",
		mcp.WithDescription("List accounts for a plan (stubbed)"),
		mcp.WithNumber("plan_id", mcp.Required(), mcp.Description("The unique identifier of the plan.")),
		mcp.WithString("sort", mcp.Description("The property to sort the results by.")),
		mcp.WithString("direction", mcp.Description("To return the oldest accounts first, set to `asc`. Ignored without the `sort` parameter.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Apps_list_accounts_for_plan_stubbedHandler(cfg),
	}
}
