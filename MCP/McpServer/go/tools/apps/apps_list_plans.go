...
func Apps_list_plansHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/marketplace_listing/plans%s", cfg.BaseURL, queryString)
...
func CreateApps_list_plansTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_marketplace_listing_plans",
		mcp.WithDescription("List plans"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Apps_list_plansHandler(cfg),
	}
}
