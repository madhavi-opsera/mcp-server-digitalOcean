...
func Apps_get_webhook_deliveryHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/app/hook/deliveries/%s", cfg.BaseURL, delivery_id)
		...
		var result models.GeneratedType_Hook_delivery
...
func CreateApps_get_webhook_deliveryTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_app_hook_deliveries_delivery_id",
		mcp.WithDescription("Get a delivery for an app webhook"),
		mcp.WithNumber("delivery_id", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Apps_get_webhook_deliveryHandler(cfg),
	}
}
