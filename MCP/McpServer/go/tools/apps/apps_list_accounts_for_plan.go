...
func Apps_list_accounts_for_planHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/marketplace_listing/plans/%s/accounts%s", cfg.BaseURL, plan_id, queryString)
...
func CreateApps_list_accounts_for_planTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_marketplace_listing_plans_plan_id_accounts",
		mcp.WithDescription("List accounts for a plan"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Apps_list_accounts_for_planHandler(cfg),
	}
}
