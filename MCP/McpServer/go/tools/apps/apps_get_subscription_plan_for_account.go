...
func Apps_get_subscription_plan_for_accountHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/marketplace_listing/accounts/%s", cfg.BaseURL, account_id)
...
func CreateApps_get_subscription_plan_for_accountTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_marketplace_listing_accounts_account_id",
		mcp.WithDescription("Get a subscription plan for an account"),
		mcp.WithNumber("account_id", mcp.Required(), mcp.Description("account_id parameter")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Apps_get_subscription_plan_for_accountHandler(cfg),
	}
}
