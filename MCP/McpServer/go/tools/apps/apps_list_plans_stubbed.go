...
func Apps_list_plans_stubbedHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/marketplace_listing/stubbed/plans%s", cfg.BaseURL, queryString)
		...
		var result []models.GeneratedType_Marketplace_listing_plan
...
func CreateApps_list_plans_stubbedTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_marketplace_listing_stubbed_plans",
		mcp.WithDescription("List plans (stubbed)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Apps_list_plans_stubbedHandler(cfg),
	}
}
