...
func Apps_list_installations_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/installations%s", cfg.BaseURL, queryString)
...
func CreateApps_list_installations_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_user_installations",
		mcp.WithDescription("List app installations accessible to the user access token"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Apps_list_installations_for_authenticated_userHandler(cfg),
	}
}
