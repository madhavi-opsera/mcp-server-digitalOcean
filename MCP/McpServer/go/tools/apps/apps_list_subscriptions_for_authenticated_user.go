...
func Apps_list_subscriptions_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/marketplace_purchases%s", cfg.BaseURL, queryString)
...
func CreateApps_list_subscriptions_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_user_marketplace_purchases",
		mcp.WithDescription("List subscriptions for the authenticated user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Apps_list_subscriptions_for_authenticated_userHandler(cfg),
	}
}
