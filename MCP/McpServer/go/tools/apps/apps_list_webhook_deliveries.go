...
func Apps_list_webhook_deliveriesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/app/hook/deliveries%s", cfg.BaseURL, queryString)
...
func CreateApps_list_webhook_deliveriesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_app_hook_deliveries",
		mcp.WithDescription("List deliveries for an app webhook"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Apps_list_webhook_deliveriesHandler(cfg),
	}
}
