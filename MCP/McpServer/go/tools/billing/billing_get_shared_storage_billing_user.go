...
func Billing_get_shared_storage_billing_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/settings/billing/shared-storage", cfg.BaseURL, username)
		...
		var result models.GeneratedType_Combined_billing_usage
...
func CreateBilling_get_shared_storage_billing_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_username_settings_billing_shared-storage",
		mcp.WithDescription("Get shared storage billing for a user"),
		mcp.WithString("username", mcp.Required(), mcp.Description("The handle for the GitHub user account.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Billing_get_shared_storage_billing_userHandler(cfg),
	}
}
