...
func Billing_get_github_billing_usage_report_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/settings/billing/usage%s", cfg.BaseURL, username, queryString)
		...
		var result models.GeneratedType_Billing_usage_report_user
...
func CreateBilling_get_github_billing_usage_report_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_username_settings_billing_usage",
		mcp.WithDescription("Get billing usage report for a user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Billing_get_github_billing_usage_report_userHandler(cfg),
	}
}
