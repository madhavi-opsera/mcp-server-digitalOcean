...
func Billing_get_github_packages_billing_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/settings/billing/packages", cfg.BaseURL, username)
		...
		var result models.GeneratedType_Packages_billing_usage
...
func CreateBilling_get_github_packages_billing_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_username_settings_billing_packages",
		mcp.WithDescription("Get GitHub Packages billing for a user"),
		mcp.WithString("username", mcp.Required(), mcp.Description("The handle for the GitHub user account.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Billing_get_github_packages_billing_userHandler(cfg),
	}
}
