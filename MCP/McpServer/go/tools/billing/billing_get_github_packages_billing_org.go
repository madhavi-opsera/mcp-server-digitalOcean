...
func Billing_get_github_packages_billing_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/settings/billing/packages", cfg.BaseURL, org)
...
func CreateBilling_get_github_packages_billing_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_settings_billing_packages",
		mcp.WithDescription("Get GitHub Packages billing for an organization"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Billing_get_github_packages_billing_orgHandler(cfg),
	}
}
