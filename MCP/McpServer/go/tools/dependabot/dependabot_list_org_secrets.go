...
func Dependabot_list_org_secretsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/dependabot/secrets%s", cfg.BaseURL, org, queryString)
...
func CreateDependabot_list_org_secretsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_dependabot_secrets",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Dependabot_list_org_secretsHandler(cfg),
	}
}
