...
func Dependabot_get_org_public_keyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/dependabot/secrets/public-key", cfg.BaseURL, org)
...
func CreateDependabot_get_org_public_keyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_dependabot_secrets_public-key",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Dependabot_get_org_public_keyHandler(cfg),
	}
}
