...
func Dependabot_get_repo_public_keyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/dependabot/secrets/public-key", cfg.BaseURL, owner, repo)
		...
		var result models.GeneratedType_Dependabot_public_key
...
func CreateDependabot_get_repo_public_keyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_dependabot_secrets_public-key",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Dependabot_get_repo_public_keyHandler(cfg),
	}
}
