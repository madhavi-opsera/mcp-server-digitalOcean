...
func Gists_list_commentsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGists_list_commentsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_gists_gist_id_comments",
		mcp.WithDescription("List gist comments"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Gists_list_commentsHandler(cfg),
	}
}
