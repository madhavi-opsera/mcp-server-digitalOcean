...
func Gists_list_starredHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/gists/starred%s", cfg.BaseURL, queryString)
...
func CreateGists_list_starredTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_gists_starred",
		mcp.WithDescription("List starred gists"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Gists_list_starredHandler(cfg),
	}
}
