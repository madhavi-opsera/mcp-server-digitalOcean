...
func Gists_create_commentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/gists/%s/comments", cfg.BaseURL, gist_id)
		...
		var result models.GeneratedType_Gist_comment
...
func CreateGists_create_commentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_gists_gist_id_comments",
		mcp.WithDescription("Create a gist comment"),
		mcp.WithString("gist_id", mcp.Required(), mcp.Description("The unique identifier of the gist.")),
		mcp.WithString("body", mcp.Required(), mcp.Description("Input parameter: The comment text.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Gists_create_commentHandler(cfg),
	}
}
