...
func Gists_forkHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/gists/%s/forks", cfg.BaseURL, gist_id)
...
func CreateGists_forkTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_gists_gist_id_forks",
		mcp.WithDescription("Fork a gist"),
		mcp.WithString("gist_id", mcp.Required(), mcp.Description("The unique identifier of the gist.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Gists_forkHandler(cfg),
	}
}
