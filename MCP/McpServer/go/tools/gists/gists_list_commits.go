...
func Gists_list_commitsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/gists/%s/commits%s", cfg.BaseURL, gist_id, queryString)
		...
		var result []models.GeneratedType_Gist_commit
...
func CreateGists_list_commitsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_gists_gist_id_commits",
		mcp.WithDescription("List gist commits"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Gists_list_commitsHandler(cfg),
	}
}
