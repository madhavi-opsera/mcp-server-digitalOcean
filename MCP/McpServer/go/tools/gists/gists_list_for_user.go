...
func Gists_list_for_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/gists%s", cfg.BaseURL, username, queryString)
		...
		var result []models.GeneratedType_Base_gist
...
func CreateGists_list_for_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_username_gists",
		mcp.WithDescription("List gists for a user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Gists_list_for_userHandler(cfg),
	}
}
