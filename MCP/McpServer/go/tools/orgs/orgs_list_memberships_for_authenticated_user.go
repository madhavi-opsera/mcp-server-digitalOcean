...
func Orgs_list_memberships_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/memberships/orgs%s", cfg.BaseURL, queryString)
		...
		var result []models.GeneratedType_Org_membership
...
func CreateOrgs_list_memberships_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_user_memberships_orgs",
		mcp.WithDescription("List organization memberships for the authenticated user"),
		mcp.WithString("state", mcp.Description("Indicates the state of the memberships to return. If not specified, the API returns both active and pending memberships.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_list_memberships_for_authenticated_userHandler(cfg),
	}
}
