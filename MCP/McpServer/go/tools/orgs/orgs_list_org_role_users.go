...
func Orgs_list_org_role_usersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/organization-roles/%s/users%s", cfg.BaseURL, org, role_id, queryString)
		...
		var result []models.GeneratedType_User_role_assignment
...
func CreateOrgs_list_org_role_usersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_organization-roles_role_id_users",
		mcp.WithDescription("List users that are assigned to an organization role"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_list_org_role_usersHandler(cfg),
	}
}
