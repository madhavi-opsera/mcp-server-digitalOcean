...
func Orgs_list_app_installationsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/installations%s", cfg.BaseURL, org, queryString)
...
func CreateOrgs_list_app_installationsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_installations",
		mcp.WithDescription("List app installations for an organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_list_app_installationsHandler(cfg),
	}
}
