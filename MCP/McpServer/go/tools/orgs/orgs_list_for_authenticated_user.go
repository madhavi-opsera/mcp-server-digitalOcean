...
func Orgs_list_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/orgs%s", cfg.BaseURL, queryString)
		...
		var result []models.GeneratedType_Organization_simple
...
func CreateOrgs_list_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_user_orgs",
		mcp.WithDescription("List organizations for the authenticated user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_list_for_authenticated_userHandler(cfg),
	}
}
