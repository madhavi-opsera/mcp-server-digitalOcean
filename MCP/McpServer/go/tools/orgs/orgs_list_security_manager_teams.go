...
func Orgs_list_security_manager_teamsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/security-managers", cfg.BaseURL, org)
		...
		var result []models.GeneratedType_Team_simple
...
func CreateOrgs_list_security_manager_teamsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_security-managers",
		mcp.WithDescription("List security manager teams"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Orgs_list_security_manager_teamsHandler(cfg),
	}
}
