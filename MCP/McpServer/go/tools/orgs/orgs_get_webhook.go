...
func Orgs_get_webhookHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateOrgs_get_webhookTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_hooks_hook_id",
		mcp.WithDescription("Get an organization webhook"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_get_webhookHandler(cfg),
	}
}
