...
func Orgs_list_public_membersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/public_members%s", cfg.BaseURL, org, queryString)
...
func CreateOrgs_list_public_membersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_public_members",
		mcp.WithDescription("List public organization members"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_list_public_membersHandler(cfg),
	}
}
