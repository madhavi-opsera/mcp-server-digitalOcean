...
func Orgs_list_membersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/members%s", cfg.BaseURL, org, queryString)
...
func CreateOrgs_list_membersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_members",
		mcp.WithDescription("List organization members"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
		mcp.WithString("filter", mcp.Description("Filter members returned in the list. `2fa_disabled` means that only members without [two-factor authentication](https://github.com/blog/1614-two-factor-authentication) enabled will be returned. `2fa_insecure` means that only members with [insecure 2FA methods](https://docs.github.com/organizations/keeping-your-organization-secure/managing-two-factor-authentication-for-your-organization/requiring-two-factor-authentication-in-your-organization#requiring-secure-methods-of-two-factor-authentication-in-your-organization) will be returned. These options are only available for organization owners.")),
		mcp.WithString("role", mcp.Description("Filter members returned by their role.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_list_membersHandler(cfg),
	}
}
