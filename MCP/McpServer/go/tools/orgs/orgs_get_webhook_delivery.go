...
func Orgs_get_webhook_deliveryHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/hooks/%s/deliveries/%s", cfg.BaseURL, org, hook_id, delivery_id)
...
func CreateOrgs_get_webhook_deliveryTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_hooks_hook_id_deliveries_delivery_id",
		mcp.WithDescription("Get a webhook delivery for an organization webhook"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_get_webhook_deliveryHandler(cfg),
	}
}
