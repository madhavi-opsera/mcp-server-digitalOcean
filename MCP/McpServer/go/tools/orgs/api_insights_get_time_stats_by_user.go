...
func Api_insights_get_time_stats_by_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/insights/api/time-stats/users/%s%s", cfg.BaseURL, org, user_id, queryString)
...
func CreateApi_insights_get_time_stats_by_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_insights_api_time-stats_users_user_id",
		mcp.WithDescription("Get time stats by user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Api_insights_get_time_stats_by_userHandler(cfg),
	}
}
