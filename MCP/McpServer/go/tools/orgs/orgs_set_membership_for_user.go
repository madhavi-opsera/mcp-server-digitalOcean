...
func Orgs_set_membership_for_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/memberships/%s", cfg.BaseURL, org, username)
...
func CreateOrgs_set_membership_for_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_orgs_org_memberships_username",
		mcp.WithDescription("Set organization membership for a user"),
		...
		mcp.WithString("role", mcp.Description("Input parameter: The role to give the user in the organization. Can be one of:  \n * `admin` - The user will become an owner of the organization.  \n * `member` - The user will become a non-owner member of the organization.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Orgs_set_membership_for_userHandler(cfg),
	}
}
