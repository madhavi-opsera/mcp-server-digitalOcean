...
func Orgs_list_for_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/orgs%s", cfg.BaseURL, username, queryString)
...
func CreateOrgs_list_for_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_username_orgs",
		mcp.WithDescription("List organizations for a user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_list_for_userHandler(cfg),
	}
}
