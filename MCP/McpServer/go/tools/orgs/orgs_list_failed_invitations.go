...
func Orgs_list_failed_invitationsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/failed_invitations%s", cfg.BaseURL, org, queryString)
...
func CreateOrgs_list_failed_invitationsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_failed_invitations",
		mcp.WithDescription("List failed organization invitations"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_list_failed_invitationsHandler(cfg),
	}
}
