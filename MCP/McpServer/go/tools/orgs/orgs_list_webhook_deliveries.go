...
func Orgs_list_webhook_deliveriesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/hooks/%s/deliveries%s", cfg.BaseURL, org, hook_id, queryString)
...
func CreateOrgs_list_webhook_deliveriesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_hooks_hook_id_deliveries",
		mcp.WithDescription("List deliveries for an organization webhook"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_list_webhook_deliveriesHandler(cfg),
	}
}
