...
func Orgs_assign_user_to_org_roleHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/organization-roles/users/%s/%s", cfg.BaseURL, org, username, role_id)
...
func CreateOrgs_assign_user_to_org_roleTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_orgs_org_organization-roles_users_username_role_id",
		mcp.WithDescription("Assign an organization role to a user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_assign_user_to_org_roleHandler(cfg),
	}
}
