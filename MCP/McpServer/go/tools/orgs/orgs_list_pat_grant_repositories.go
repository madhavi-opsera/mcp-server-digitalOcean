...
func Orgs_list_pat_grant_repositoriesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		pat_idVal, ok := args["pat_id"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: pat_id"), nil
		}
		pat_id, ok := pat_idVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: pat_id"), nil
		}
		...
		url := fmt.Sprintf("%s/orgs/%s/personal-access-tokens/%s/repositories%s", cfg.BaseURL, org, pat_id, queryString)
		...
		var result []models.GeneratedType_Minimal_repository
...
func CreateOrgs_list_pat_grant_repositoriesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_personal-access-tokens_pat_id_repositories",
		mcp.WithDescription("List repositories a fine-grained personal access token has access to"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
		mcp.WithNumber("pat_id", mcp.Required(), mcp.Description("Unique identifier of the fine-grained personal access token.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_list_pat_grant_repositoriesHandler(cfg),
	}
}
