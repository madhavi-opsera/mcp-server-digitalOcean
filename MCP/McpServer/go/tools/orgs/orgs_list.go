...
func Orgs_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/organizations%s", cfg.BaseURL, queryString)
...
func CreateOrgs_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_organizations",
		mcp.WithDescription("List organizations"),
		mcp.WithNumber("since", mcp.Description("An organization ID. Only return organizations with an ID greater than this ID.")),
