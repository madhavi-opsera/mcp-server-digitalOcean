...
func Classroom_get_an_assignmentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/assignments/%s", cfg.BaseURL, assignment_id)
		...
		var result models.GeneratedType_Classroom_assignment
...
func CreateClassroom_get_an_assignmentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_assignments_assignment_id",
		mcp.WithDescription("Get an assignment"),
		mcp.WithNumber("assignment_id", mcp.Required(), mcp.Description("The unique identifier of the classroom assignment.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Classroom_get_an_assignmentHandler(cfg),
	}
}
