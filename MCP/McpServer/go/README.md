...
{
  "mcpServers": {
    "GitHub v3 REST API-mcp-server-stdio": {
...
```bash
docker build -t GitHub v3 REST API-mcp-server .
```
...
{
  "mcpServers": {
    "GitHub v3 REST API-mcp-server-docker": {
      "command": "docker",
      "args": [
        "run", "--rm", "-i",
        "-p", "8080:8080",
...
```bash
docker run -d \
  --name GitHub v3 REST API-mcp-server \
  -p 8080:8080 \
...
{
  "mcpServers": {
    "GitHub v3 REST API-mcp-server-http": {
...
{
  "mcpServers": {
    "GitHub v3 REST API-mcp-server-https": {
