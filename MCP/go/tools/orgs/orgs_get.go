...
func Orgs_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateOrgs_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org",
		mcp.WithDescription("Get an organization"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Orgs_getHandler(cfg),
	}
}
