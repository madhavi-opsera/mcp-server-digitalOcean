...
func Orgs_get_all_custom_propertiesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateOrgs_get_all_custom_propertiesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_properties_schema",
		mcp.WithDescription("Get all custom properties for an organization"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Orgs_get_all_custom_propertiesHandler(cfg),
	}
}
