...
func Orgs_create_or_update_custom_propertiesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/properties/schema%s", cfg.BaseURL, org, queryString)
...
func CreateOrgs_create_or_update_custom_propertiesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_orgs_org_properties_schema",
		mcp.WithDescription("Create or update custom properties for an organization"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
		mcp.WithArray("properties", mcp.Required(), mcp.Description("Input parameter: The array of custom properties to create or update.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Orgs_create_or_update_custom_propertiesHandler(cfg),
	}
}
