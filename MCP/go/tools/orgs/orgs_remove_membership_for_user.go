...
func Orgs_remove_membership_for_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateOrgs_remove_membership_for_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_orgs_org_memberships_username",
		mcp.WithDescription("Remove organization membership for a user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_remove_membership_for_userHandler(cfg),
	}
}
