...
func Orgs_convert_member_to_outside_collaboratorHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/outside_collaborators/%s%s", cfg.BaseURL, org, username, queryString)
...
func CreateOrgs_convert_member_to_outside_collaboratorTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_orgs_org_outside_collaborators_username",
		mcp.WithDescription("Convert an organization member to outside collaborator"),
		...
		mcp.WithBoolean("async", mcp.Description("Input parameter: When set to `true`, the request will be performed asynchronously. Returns a 202 status code when the job is successfully queued.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Orgs_convert_member_to_outside_collaboratorHandler(cfg),
	}
}
