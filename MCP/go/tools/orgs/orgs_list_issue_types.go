...
func Orgs_list_issue_typesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateOrgs_list_issue_typesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_issue-types",
		mcp.WithDescription("List issue types for an organization"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Orgs_list_issue_typesHandler(cfg),
	}
}
