...
func Orgs_check_blocked_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/blocks/%s%s", cfg.BaseURL, org, username, queryString)
...
func CreateOrgs_check_blocked_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_blocks_username",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_check_blocked_userHandler(cfg),
	}
}
