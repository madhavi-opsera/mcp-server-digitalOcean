...
func Orgs_check_membership_for_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/members/%s%s", cfg.BaseURL, org, username, queryString)
...
func CreateOrgs_check_membership_for_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_members_username",
		mcp.WithDescription("Check organization membership for a user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_check_membership_for_userHandler(cfg),
	}
}
