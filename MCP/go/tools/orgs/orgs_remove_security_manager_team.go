...
func Orgs_remove_security_manager_teamHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/security-managers/teams/%s%s", cfg.BaseURL, org, team_slug, queryString)
...
func CreateOrgs_remove_security_manager_teamTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_orgs_org_security-managers_teams_team_slug",
		mcp.WithDescription("Remove a security manager team"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_remove_security_manager_teamHandler(cfg),
	}
}
