...
func Orgs_remove_memberHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateOrgs_remove_memberTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_orgs_org_members_username",
		mcp.WithDescription("Remove an organization member"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_remove_memberHandler(cfg),
	}
}
