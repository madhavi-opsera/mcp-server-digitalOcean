...
func Orgs_set_public_membership_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateOrgs_set_public_membership_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_orgs_org_public_members_username",
		mcp.WithDescription("Set public organization membership for the authenticated user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_set_public_membership_for_authenticated_userHandler(cfg),
	}
}
