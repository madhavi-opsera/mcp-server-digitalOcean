...
func Orgs_get_webhook_config_for_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateOrgs_get_webhook_config_for_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_hooks_hook_id_config",
		mcp.WithDescription("Get a webhook configuration for an organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_get_webhook_config_for_orgHandler(cfg),
	}
}
