...
func Orgs_remove_public_membership_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/public_members/%s%s", cfg.BaseURL, org, username, queryString)
...
func CreateOrgs_remove_public_membership_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_orgs_org_public_members_username",
		mcp.WithDescription("Remove public organization membership for the authenticated user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_remove_public_membership_for_authenticated_userHandler(cfg),
	}
}
