...
func Orgs_list_pending_invitationsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["invitation_source"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("invitation_source=%v", val))
		}
...
func CreateOrgs_list_pending_invitationsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_invitations",
		mcp.WithDescription("List pending organization invitations"),
		...
		mcp.WithString("role", mcp.Description("Filter invitations by their member role.")),
		mcp.WithString("invitation_source", mcp.Description("Filter invitations by their invitation source.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Orgs_list_pending_invitationsHandler(cfg),
	}
}
