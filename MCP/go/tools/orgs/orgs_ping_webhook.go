...
func Orgs_ping_webhookHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/hooks/%s/pings%s", cfg.BaseURL, org, hook_id, queryString)
...
func CreateOrgs_ping_webhookTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_orgs_org_hooks_hook_id_pings",
		mcp.WithDescription("Ping an organization webhook"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_ping_webhookHandler(cfg),
	}
}
