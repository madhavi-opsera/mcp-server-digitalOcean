...
func Orgs_remove_outside_collaboratorHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateOrgs_remove_outside_collaboratorTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_orgs_org_outside_collaborators_username",
		mcp.WithDescription("Remove outside collaborator from an organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_remove_outside_collaboratorHandler(cfg),
	}
}
