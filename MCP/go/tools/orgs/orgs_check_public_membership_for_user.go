...
func Orgs_check_public_membership_for_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateOrgs_check_public_membership_for_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_public_members_username",
		mcp.WithDescription("Check public organization membership for a user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_check_public_membership_for_userHandler(cfg),
	}
}
