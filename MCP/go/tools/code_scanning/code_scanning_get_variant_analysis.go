...
func Code_scanning_get_variant_analysisHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/code-scanning/codeql/variant-analyses/%s%s", cfg.BaseURL, owner, repo, codeql_variant_analysis_id, queryString)
...
func CreateCode_scanning_get_variant_analysisTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_code-scanning_codeql_variant-analyses_codeql_variant_analysis_id",
		mcp.WithDescription("Get the summary of a CodeQL variant analysis"),
		...
		mcp.WithNumber("codeql_variant_analysis_id", mcp.Required(), mcp.Description("The unique identifier of the variant analysis.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Code_scanning_get_variant_analysisHandler(cfg),
	}
}
