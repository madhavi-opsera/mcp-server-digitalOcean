...
func Apps_suspend_installationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateApps_suspend_installationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_app_installations_installation_id_suspended",
		mcp.WithDescription("Suspend an app installation"),
		mcp.WithNumber("installation_id", mcp.Required(), mcp.Description("The unique identifier of the installation.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Apps_suspend_installationHandler(cfg),
	}
}
