...
func Apps_get_repo_installationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/installation%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateApps_get_repo_installationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_installation",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Apps_get_repo_installationHandler(cfg),
	}
}
