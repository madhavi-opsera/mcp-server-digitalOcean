...
func Apps_reset_tokenHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/applications/%s/token%s", cfg.BaseURL, client_id, queryString)
...
func CreateApps_reset_tokenTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_applications_client_id_token",
		mcp.WithDescription("Reset a token"),
		mcp.WithString("client_id", mcp.Required(), mcp.Description("The client ID of the GitHub app.")),
		mcp.WithString("access_token", mcp.Required(), mcp.Description("Input parameter: The access_token of the OAuth or GitHub application.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Apps_reset_tokenHandler(cfg),
	}
}
