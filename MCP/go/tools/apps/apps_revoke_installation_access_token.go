...
func Apps_revoke_installation_access_tokenHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/installation/token%s", cfg.BaseURL, queryString)
...
func CreateApps_revoke_installation_access_tokenTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_installation_token",
		mcp.WithDescription("Revoke an installation access token"),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Apps_revoke_installation_access_tokenHandler(cfg),
	}
}
