...
func Apps_get_authenticatedHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/app%s", cfg.BaseURL, queryString)
...
func CreateApps_get_authenticatedTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_app",
		mcp.WithDescription("Get the authenticated app"),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Apps_get_authenticatedHandler(cfg),
	}
}
