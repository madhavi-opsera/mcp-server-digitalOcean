...
func Apps_get_installationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateApps_get_installationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_app_installations_installation_id",
		mcp.WithDescription("Get an installation for the authenticated app"),
		mcp.WithNumber("installation_id", mcp.Required(), mcp.Description("The unique identifier of the installation.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Apps_get_installationHandler(cfg),
	}
}
