...
func Apps_list_repos_accessible_to_installationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/installation/repositories%s", cfg.BaseURL, queryString)
...
func CreateApps_list_repos_accessible_to_installationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_installation_repositories",
		mcp.WithDescription("List repositories accessible to the app installation"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Apps_list_repos_accessible_to_installationHandler(cfg),
	}
}
