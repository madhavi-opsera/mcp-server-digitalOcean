...
func Apps_get_webhook_config_for_appHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateApps_get_webhook_config_for_appTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_app_hook_config",
		mcp.WithDescription("Get a webhook configuration for an app"),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Apps_get_webhook_config_for_appHandler(cfg),
	}
}
