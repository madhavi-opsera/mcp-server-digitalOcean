...
func Apps_add_repo_to_installation_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/installations/%s/repositories/%s%s", cfg.BaseURL, installation_id, repository_id, queryString)
...
func CreateApps_add_repo_to_installation_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_user_installations_installation_id_repositories_repository_id",
		mcp.WithDescription("Add a repository to an app installation"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Apps_add_repo_to_installation_for_authenticated_userHandler(cfg),
	}
}
