...
func Apps_unsuspend_installationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/app/installations/%s/suspended%s", cfg.BaseURL, installation_id, queryString)
...
func CreateApps_unsuspend_installationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_app_installations_installation_id_suspended",
		mcp.WithDescription("Unsuspend an app installation"),
		mcp.WithNumber("installation_id", mcp.Required(), mcp.Description("The unique identifier of the installation.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Apps_unsuspend_installationHandler(cfg),
	}
}
