...
func Apps_get_user_installationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/installation%s", cfg.BaseURL, username, queryString)
...
func CreateApps_get_user_installationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_username_installation",
		mcp.WithDescription("Get a user installation for the authenticated app"),
		mcp.WithString("username", mcp.Required(), mcp.Description("The handle for the GitHub user account.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Apps_get_user_installationHandler(cfg),
	}
}
