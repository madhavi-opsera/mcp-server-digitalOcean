...
func Apps_delete_authorizationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/applications/%s/grant%s", cfg.BaseURL, client_id, queryString)
...
func CreateApps_delete_authorizationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_applications_client_id_grant",
		mcp.WithDescription("Delete an app authorization"),
		mcp.WithString("client_id", mcp.Required(), mcp.Description("The client ID of the GitHub app.")),
		mcp.WithString("access_token", mcp.Required(), mcp.Description("Input parameter: The OAuth access token used to authenticate to the GitHub API.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Apps_delete_authorizationHandler(cfg),
	}
}
