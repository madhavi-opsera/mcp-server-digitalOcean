...
func Apps_check_tokenHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateApps_check_tokenTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_applications_client_id_token",
		mcp.WithDescription("Check a token"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Apps_check_tokenHandler(cfg),
	}
}
