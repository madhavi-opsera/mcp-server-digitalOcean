...
func Checks_rerequest_runHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/check-runs/%s/rerequest%s", cfg.BaseURL, owner, repo, check_run_id, queryString)
...
func CreateChecks_rerequest_runTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_check-runs_check_run_id_rerequest",
		mcp.WithDescription("Rerequest a check run"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Checks_rerequest_runHandler(cfg),
	}
}
