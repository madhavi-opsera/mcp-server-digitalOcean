...
func Checks_list_suites_for_refHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/commits/%s/check-suites%s", cfg.BaseURL, owner, repo, ref, queryString)
...
func CreateChecks_list_suites_for_refTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_commits_ref_check-suites",
		mcp.WithDescription("List check suites for a Git reference"),
		...
		mcp.WithNumber("app_id", mcp.Description("Filters check suites by GitHub App `id`.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Checks_list_suites_for_refHandler(cfg),
	}
}
