...
func Credentials_revokeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/credentials/revoke%s", cfg.BaseURL, queryString)
...
func CreateCredentials_revokeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_credentials_revoke",
		mcp.WithDescription("Revoke a list of credentials"),
		mcp.WithArray("credentials", mcp.Required(), mcp.Description("Input parameter: A list of credentials to be revoked, up to 1000 per request.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Credentials_revokeHandler(cfg),
	}
}
