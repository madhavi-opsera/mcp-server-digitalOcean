...
func Repos_check_collaboratorHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/collaborators/%s%s", cfg.BaseURL, owner, repo, username, queryString)
...
func CreateRepos_check_collaboratorTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_collaborators_username",
		mcp.WithDescription("Check if a user is a repository collaborator"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_check_collaboratorHandler(cfg),
	}
}
