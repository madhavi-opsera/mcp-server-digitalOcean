...
func Repos_create_deployment_protection_ruleHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/environments/%s/deployment_protection_rules%s", cfg.BaseURL, environment_name, repo, owner, queryString)
...
func CreateRepos_create_deployment_protection_ruleTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_environments_environment_name_deployment_protection_rules",
		mcp.WithDescription("Create a custom deployment protection rule on an environment"),
		...
		mcp.WithNumber("integration_id", mcp.Description("Input parameter: The ID of the custom app that will be enabled on the environment.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_create_deployment_protection_ruleHandler(cfg),
	}
}
