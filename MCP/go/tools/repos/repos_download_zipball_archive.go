...
func Repos_download_zipball_archiveHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/zipball/%s%s", cfg.BaseURL, owner, repo, ref, queryString)
...
func CreateRepos_download_zipball_archiveTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_zipball_ref",
		mcp.WithDescription("Download a repository archive (zip)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_download_zipball_archiveHandler(cfg),
	}
}
