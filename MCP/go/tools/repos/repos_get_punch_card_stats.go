...
func Repos_get_punch_card_statsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/stats/punch_card%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateRepos_get_punch_card_statsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_stats_punch_card",
		mcp.WithDescription("Get the hourly commit count for each day"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_punch_card_statsHandler(cfg),
	}
}
