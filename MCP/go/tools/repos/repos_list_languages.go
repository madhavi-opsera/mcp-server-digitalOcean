...
func Repos_list_languagesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/languages%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateRepos_list_languagesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_languages",
		mcp.WithDescription("List repository languages"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_languagesHandler(cfg),
	}
}
