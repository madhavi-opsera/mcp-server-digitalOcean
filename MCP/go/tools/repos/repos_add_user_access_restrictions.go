...
func Repos_add_user_access_restrictionsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_add_user_access_restrictionsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_branches_branch_protection_restrictions_users",
		mcp.WithDescription("Add user access restrictions"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_add_user_access_restrictionsHandler(cfg),
	}
}
