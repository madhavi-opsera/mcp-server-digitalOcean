...
func Repos_list_tagsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/tags%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateRepos_list_tagsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_tags",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_tagsHandler(cfg),
	}
}
