...
func Repos_disable_automated_security_fixesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_disable_automated_security_fixesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_automated-security-fixes",
		mcp.WithDescription("Disable Dependabot security updates"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_disable_automated_security_fixesHandler(cfg),
	}
}
