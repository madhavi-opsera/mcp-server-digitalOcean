...
func Repos_list_teamsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/teams%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateRepos_list_teamsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_teams",
		mcp.WithDescription("List repository teams"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_teamsHandler(cfg),
	}
}
