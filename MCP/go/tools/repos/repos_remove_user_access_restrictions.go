...
func Repos_remove_user_access_restrictionsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/branches/%s/protection/restrictions/users%s", cfg.BaseURL, owner, repo, branch, queryString)
...
func CreateRepos_remove_user_access_restrictionsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_branches_branch_protection_restrictions_users",
		mcp.WithDescription("Remove user access restrictions"),
		...
		mcp.WithArray("users", mcp.Required(), mcp.Description("Input parameter: The username for users")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_remove_user_access_restrictionsHandler(cfg),
	}
}
