...
func Repos_remove_app_access_restrictionsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/branches/%s/protection/restrictions/apps%s", cfg.BaseURL, owner, repo, branch, queryString)
...
func CreateRepos_remove_app_access_restrictionsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_branches_branch_protection_restrictions_apps",
		mcp.WithDescription("Remove app access restrictions"),
		...
		mcp.WithArray("apps", mcp.Required(), mcp.Description("Input parameter: The GitHub Apps that have push access to this branch. Use the slugified version of the app name. **Note**: The list of users, apps, and teams in total is limited to 100 items.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_remove_app_access_restrictionsHandler(cfg),
	}
}
