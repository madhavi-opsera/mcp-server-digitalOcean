...
func Repos_delete_releaseHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_delete_releaseTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_releases_release_id",
		mcp.WithDescription("Delete a release"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_delete_releaseHandler(cfg),
	}
}
