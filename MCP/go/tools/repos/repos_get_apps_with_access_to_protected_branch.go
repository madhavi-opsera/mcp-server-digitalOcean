...
func Repos_get_apps_with_access_to_protected_branchHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_get_apps_with_access_to_protected_branchTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_branches_branch_protection_restrictions_apps",
		mcp.WithDescription("Get apps with access to the protected branch"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_apps_with_access_to_protected_branchHandler(cfg),
	}
}
