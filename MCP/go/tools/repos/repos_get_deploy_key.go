...
func Repos_get_deploy_keyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_get_deploy_keyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_keys_key_id",
		mcp.WithDescription("Get a deploy key"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_deploy_keyHandler(cfg),
	}
}
