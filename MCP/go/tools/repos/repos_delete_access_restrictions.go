...
func Repos_delete_access_restrictionsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/branches/%s/protection/restrictions%s", cfg.BaseURL, owner, repo, branch, queryString)
...
func CreateRepos_delete_access_restrictionsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_branches_branch_protection_restrictions",
		mcp.WithDescription("Delete access restrictions"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_delete_access_restrictionsHandler(cfg),
	}
}
