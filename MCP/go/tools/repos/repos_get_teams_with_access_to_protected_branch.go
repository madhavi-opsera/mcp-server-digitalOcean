...
func Repos_get_teams_with_access_to_protected_branchHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/branches/%s/protection/restrictions/teams%s", cfg.BaseURL, owner, repo, branch, queryString)
...
func CreateRepos_get_teams_with_access_to_protected_branchTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_branches_branch_protection_restrictions_teams",
		mcp.WithDescription("Get teams with access to the protected branch"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_teams_with_access_to_protected_branchHandler(cfg),
	}
}
