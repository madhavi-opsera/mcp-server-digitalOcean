...
func Repos_redeliver_webhook_deliveryHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/hooks/%s/deliveries/%s/attempts%s", cfg.BaseURL, owner, repo, hook_id, delivery_id, queryString)
...
func CreateRepos_redeliver_webhook_deliveryTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_hooks_hook_id_deliveries_delivery_id_attempts",
		mcp.WithDescription("Redeliver a delivery for a repository webhook"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_redeliver_webhook_deliveryHandler(cfg),
	}
}
