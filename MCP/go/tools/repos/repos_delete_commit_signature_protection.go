...
func Repos_delete_commit_signature_protectionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/branches/%s/protection/required_signatures%s", cfg.BaseURL, owner, repo, branch, queryString)
...
func CreateRepos_delete_commit_signature_protectionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_branches_branch_protection_required_signatures",
		mcp.WithDescription("Delete commit signature protection"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_delete_commit_signature_protectionHandler(cfg),
	}
}
