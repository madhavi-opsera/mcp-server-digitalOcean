...
func Repos_get_all_status_check_contextsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/branches/%s/protection/required_status_checks/contexts%s", cfg.BaseURL, owner, repo, branch, queryString)
...
func CreateRepos_get_all_status_check_contextsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_branches_branch_protection_required_status_checks_contexts",
		mcp.WithDescription("Get all status check contexts"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_all_status_check_contextsHandler(cfg),
	}
}
