...
func Repos_get_repo_rulesetsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["includes_parents"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("includes_parents=%v", val))
		}
		if val, ok := args["targets"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("targets=%v", val))
		}
...
func CreateRepos_get_repo_rulesetsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_rulesets",
		mcp.WithDescription("Get all repository rulesets"),
		...
		mcp.WithBoolean("includes_parents", mcp.Description("Include rulesets configured at higher levels that apply to this repository")),
		mcp.WithString("targets", mcp.Description("A comma-separated list of rule targets to filter by.\nIf provided, only rulesets that apply to the specified targets will be returned.\nFor example, `branch,tag,push`.\n")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_repo_rulesetsHandler(cfg),
	}
}
