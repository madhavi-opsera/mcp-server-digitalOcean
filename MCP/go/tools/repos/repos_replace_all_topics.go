...
func Repos_replace_all_topicsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/topics%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateRepos_replace_all_topicsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_repos_owner_repo_topics",
		mcp.WithDescription("Replace all repository topics"),
		...
		mcp.WithArray("names", mcp.Required(), mcp.Description("Input parameter: An array of topics to add to the repository. Pass one or more topics to _replace_ the set of existing topics. Send an empty array (`[]`) to clear all topics from the repository. **Note:** Topic `names` will be saved as lowercase.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_replace_all_topicsHandler(cfg),
	}
}
