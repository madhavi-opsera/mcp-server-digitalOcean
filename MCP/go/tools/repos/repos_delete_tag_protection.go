...
func Repos_delete_tag_protectionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		tag_protection_idVal, ok := args["tag_protection_id"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: tag_protection_id"), nil
		}
		tag_protection_id, ok := tag_protection_idVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: tag_protection_id"), nil
		}
		...
		url := fmt.Sprintf("%s/repos/%s/%s/tags/protection/%s%s", cfg.BaseURL, owner, repo, tag_protection_id, queryString)
...
func CreateRepos_delete_tag_protectionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_tags_protection_tag_protection_id",
		mcp.WithDescription("Closing down - Delete a tag protection state for a repository"),
		...
		mcp.WithNumber("tag_protection_id", mcp.Required(), mcp.Description("The unique identifier of the tag protection.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_delete_tag_protectionHandler(cfg),
	}
}
