...
func Repos_get_deployment_branch_policyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_get_deployment_branch_policyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_environments_environment_name_deployment-branch-policies_branch_policy_id",
		mcp.WithDescription("Get a deployment branch policy"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_deployment_branch_policyHandler(cfg),
	}
}
