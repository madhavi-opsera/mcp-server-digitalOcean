...
func Repos_create_attestationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/attestations%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateRepos_create_attestationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_attestations",
		mcp.WithDescription("Create an attestation"),
		...
		mcp.WithObject("bundle", mcp.Required(), mcp.Description("Input parameter: The attestation's Sigstore Bundle.\nRefer to the [Sigstore Bundle Specification](https://github.com/sigstore/protobuf-specs/blob/main/protos/sigstore_bundle.proto) for more information.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_create_attestationHandler(cfg),
	}
}
