...
func Repos_download_tarball_archiveHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/tarball/%s%s", cfg.BaseURL, owner, repo, ref, queryString)
...
func CreateRepos_download_tarball_archiveTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_tarball_ref",
		mcp.WithDescription("Download a repository archive (tar)"),
		...
		mcp.WithString("ref", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_download_tarball_archiveHandler(cfg),
	}
}
