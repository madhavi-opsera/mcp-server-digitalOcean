...
func Repos_delete_deployment_branch_policyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/environments/%s/deployment-branch-policies/%s%s", cfg.BaseURL, owner, repo, environment_name, branch_policy_id, queryString)
...
func CreateRepos_delete_deployment_branch_policyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_environments_environment_name_deployment-branch-policies_branch_policy_id",
		mcp.WithDescription("Delete a deployment branch policy"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_delete_deployment_branch_policyHandler(cfg),
	}
}
