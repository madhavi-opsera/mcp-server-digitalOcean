...
func Repos_delete_autolinkHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		autolink_idVal, ok := args["autolink_id"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: autolink_id"), nil
		}
		autolink_id, ok := autolink_idVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: autolink_id"), nil
		}
		...
		url := fmt.Sprintf("%s/repos/%s/%s/autolinks/%s%s", cfg.BaseURL, owner, repo, autolink_id, queryString)
...
func CreateRepos_delete_autolinkTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_autolinks_autolink_id",
		mcp.WithDescription("Delete an autolink reference from a repository"),
		...
		mcp.WithNumber("autolink_id", mcp.Required(), mcp.Description("The unique identifier of the autolink.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_delete_autolinkHandler(cfg),
	}
}
