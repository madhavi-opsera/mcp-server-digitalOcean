...
func Repos_list_pull_requests_associated_with_commitHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/commits/%s/pulls%s", cfg.BaseURL, owner, repo, commit_sha, queryString)
...
func CreateRepos_list_pull_requests_associated_with_commitTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_commits_commit_sha_pulls",
		mcp.WithDescription("List pull requests associated with a commit"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_pull_requests_associated_with_commitHandler(cfg),
	}
}
