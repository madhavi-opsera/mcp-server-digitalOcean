...
func Repos_list_webhooksHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_list_webhooksTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_hooks",
		mcp.WithDescription("List repository webhooks"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_webhooksHandler(cfg),
	}
}
