...
func Repos_list_deploy_keysHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_list_deploy_keysTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_keys",
		mcp.WithDescription("List deploy keys"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_deploy_keysHandler(cfg),
	}
}
