...
func Repos_delete_commit_commentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/comments/%s%s", cfg.BaseURL, owner, repo, comment_id, queryString)
...
func CreateRepos_delete_commit_commentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_comments_comment_id",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_delete_commit_commentHandler(cfg),
	}
}
