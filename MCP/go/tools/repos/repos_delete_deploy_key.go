...
func Repos_delete_deploy_keyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/keys/%s%s", cfg.BaseURL, owner, repo, key_id, queryString)
...
func CreateRepos_delete_deploy_keyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_keys_key_id",
		mcp.WithDescription("Delete a deploy key"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_delete_deploy_keyHandler(cfg),
	}
}
