...
func Repos_ping_webhookHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/hooks/%s/pings%s", cfg.BaseURL, owner, repo, hook_id, queryString)
...
func CreateRepos_ping_webhookTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_hooks_hook_id_pings",
		mcp.WithDescription("Ping a repository webhook"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_ping_webhookHandler(cfg),
	}
}
