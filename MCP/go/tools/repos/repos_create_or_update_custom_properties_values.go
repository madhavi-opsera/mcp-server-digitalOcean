...
func Repos_create_or_update_custom_properties_valuesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/properties/values%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateRepos_create_or_update_custom_properties_valuesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_repos_owner_repo_properties_values",
		mcp.WithDescription("Create or update custom property values for a repository"),
		...
		mcp.WithArray("properties", mcp.Required(), mcp.Description("Input parameter: A list of custom property names and associated values to apply to the repositories.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_create_or_update_custom_properties_valuesHandler(cfg),
	}
}
