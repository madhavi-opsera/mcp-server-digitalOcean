...
func Repos_get_latest_releaseHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/releases/latest%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateRepos_get_latest_releaseTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_releases_latest",
		mcp.WithDescription("Get the latest release"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_latest_releaseHandler(cfg),
	}
}
