...
func Repos_list_release_assetsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/releases/%s/assets%s", cfg.BaseURL, owner, repo, release_id, queryString)
...
func CreateRepos_list_release_assetsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_releases_release_id_assets",
		mcp.WithDescription("List release assets"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_release_assetsHandler(cfg),
	}
}
