...
func Repos_test_push_webhookHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/hooks/%s/tests%s", cfg.BaseURL, owner, repo, hook_id, queryString)
...
func CreateRepos_test_push_webhookTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_hooks_hook_id_tests",
		mcp.WithDescription("Test the push repository webhook"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_test_push_webhookHandler(cfg),
	}
}
