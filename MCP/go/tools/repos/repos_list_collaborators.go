...
func Repos_list_collaboratorsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/collaborators%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateRepos_list_collaboratorsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_collaborators",
		mcp.WithDescription("List repository collaborators"),
		...
		mcp.WithString("affiliation", mcp.Description("Filter collaborators returned by their affiliation. `outside` means all outside collaborators of an organization-owned repository. `direct` means all collaborators with permissions to an organization-owned repository, regardless of organization membership status. `all` means all collaborators the authenticated user can see.")),
		mcp.WithString("permission", mcp.Description("Filter collaborators by the permissions they have on the repository. If not specified, all collaborators will be returned.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_collaboratorsHandler(cfg),
	}
}
