...
func Repos_list_attestationsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		subject_digestVal, ok := args["subject_digest"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: subject_digest"), nil
		}
		subject_digest, ok := subject_digestVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: subject_digest"), nil
		}
		...
		if val, ok := args["predicate_type"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("predicate_type=%v", val))
		}
		...
		url := fmt.Sprintf("%s/repos/%s/%s/attestations/%s%s", cfg.BaseURL, owner, repo, subject_digest, queryString)
...
func CreateRepos_list_attestationsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_attestations_subject_digest",
		mcp.WithDescription("List attestations"),
		...
		mcp.WithString("subject_digest", mcp.Required(), mcp.Description("The parameter should be set to the attestation's subject's SHA256 digest, in the form `sha256:HEX_DIGEST`.")),
		mcp.WithString("predicate_type", mcp.Description("Optional filter for fetching attestations with a given predicate type.\nThis option accepts `provenance`, `sbom`, or freeform text for custom predicate types.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_list_attestationsHandler(cfg),
	}
}
