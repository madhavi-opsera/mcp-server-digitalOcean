...
func Repos_get_deployment_statusHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		status_idVal, ok := args["status_id"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: status_id"), nil
		}
		status_id, ok := status_idVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: status_id"), nil
		}
		...
		url := fmt.Sprintf("%s/repos/%s/%s/deployments/%s/statuses/%s%s", cfg.BaseURL, owner, repo, deployment_id, status_id, queryString)
...
func CreateRepos_get_deployment_statusTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_deployments_deployment_id_statuses_status_id",
		mcp.WithDescription("Get a deployment status"),
		...
		mcp.WithNumber("status_id", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_deployment_statusHandler(cfg),
	}
}
