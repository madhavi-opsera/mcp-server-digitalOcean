...
func Repos_remove_status_check_protectionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_remove_status_check_protectionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_branches_branch_protection_required_status_checks",
		mcp.WithDescription("Remove status check protection"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_remove_status_check_protectionHandler(cfg),
	}
}
