...
func Repos_get_release_by_tagHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		tagVal, ok := args["tag"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: tag"), nil
		}
		tag, ok := tagVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: tag"), nil
		}
		...
		url := fmt.Sprintf("%s/repos/%s/%s/releases/tags/%s%s", cfg.BaseURL, owner, repo, tag, queryString)
...
func CreateRepos_get_release_by_tagTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_releases_tags_tag",
		...
		mcp.WithString("tag", mcp.Required(), mcp.Description("tag parameter")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_release_by_tagHandler(cfg),
	}
}
