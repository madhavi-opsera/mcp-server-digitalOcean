...
func Repos_remove_collaboratorHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_remove_collaboratorTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_collaborators_username",
		mcp.WithDescription("Remove a repository collaborator"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_remove_collaboratorHandler(cfg),
	}
}
