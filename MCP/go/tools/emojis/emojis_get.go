...
func Emojis_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/emojis%s", cfg.BaseURL, queryString)
...
func CreateEmojis_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_emojis",
		mcp.WithDescription("Get emojis"),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Emojis_getHandler(cfg),
	}
}
