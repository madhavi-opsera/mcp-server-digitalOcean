...
func Issues_remove_all_labelsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/issues/%s/labels%s", cfg.BaseURL, owner, repo, issue_number, queryString)
...
func CreateIssues_remove_all_labelsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_issues_issue_number_labels",
		mcp.WithDescription("Remove all labels from an issue"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Issues_remove_all_labelsHandler(cfg),
	}
}
