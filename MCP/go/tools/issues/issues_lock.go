...
func Issues_lockHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/issues/%s/lock%s", cfg.BaseURL, owner, repo, issue_number, queryString)
...
func CreateIssues_lockTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_repos_owner_repo_issues_issue_number_lock",
		mcp.WithDescription("Lock an issue"),
		...
		mcp.WithString("lock_reason", mcp.Description("Input parameter: The reason for locking the issue or pull request conversation. Lock will fail if you don't use one of these reasons:  \n * `off-topic`  \n * `too heated`  \n * `resolved`  \n * `spam`")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Issues_lockHandler(cfg),
	}
}
