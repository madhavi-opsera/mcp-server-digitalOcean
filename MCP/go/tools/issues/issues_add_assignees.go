...
func Issues_add_assigneesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateIssues_add_assigneesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_issues_issue_number_assignees",
		mcp.WithDescription("Add assignees to an issue"),
		...
		mcp.WithArray("assignees", mcp.Description("Input parameter: Usernames of people to assign this issue to. _NOTE: Only users with push access can add assignees to an issue. Assignees are silently ignored otherwise._")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Issues_add_assigneesHandler(cfg),
	}
}
