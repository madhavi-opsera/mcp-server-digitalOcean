...
func Issues_remove_sub_issueHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/issues/%s/sub_issue%s", cfg.BaseURL, owner, repo, issue_number, queryString)
...
func CreateIssues_remove_sub_issueTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_issues_issue_number_sub_issue",
		mcp.WithDescription("Remove sub-issue"),
		...
		mcp.WithNumber("sub_issue_id", mcp.Required(), mcp.Description("Input parameter: The id of the sub-issue to remove")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Issues_remove_sub_issueHandler(cfg),
	}
}
