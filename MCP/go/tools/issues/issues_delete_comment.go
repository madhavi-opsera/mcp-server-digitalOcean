...
func Issues_delete_commentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/issues/comments/%s%s", cfg.BaseURL, owner, repo, comment_id, queryString)
...
func CreateIssues_delete_commentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_issues_comments_comment_id",
		mcp.WithDescription("Delete an issue comment"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Issues_delete_commentHandler(cfg),
	}
}
