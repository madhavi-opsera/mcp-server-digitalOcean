...
func Issues_list_labels_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateIssues_list_labels_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_labels",
		mcp.WithDescription("List labels for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Issues_list_labels_for_repoHandler(cfg),
	}
}
