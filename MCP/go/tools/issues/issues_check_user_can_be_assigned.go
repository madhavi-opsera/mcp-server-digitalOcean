...
func Issues_check_user_can_be_assignedHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/assignees/%s%s", cfg.BaseURL, owner, repo, assignee, queryString)
...
func CreateIssues_check_user_can_be_assignedTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_assignees_assignee",
		mcp.WithDescription("Check if a user can be assigned"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Issues_check_user_can_be_assignedHandler(cfg),
	}
}
