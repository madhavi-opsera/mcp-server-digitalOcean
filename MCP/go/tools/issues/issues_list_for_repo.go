...
func Issues_list_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["milestone"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("milestone=%v", val))
		}
		...
		if val, ok := args["creator"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("creator=%v", val))
		}
		if val, ok := args["mentioned"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("mentioned=%v", val))
		}
		if val, ok := args["labels"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("labels=%v", val))
		}
...
func CreateIssues_list_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_issues",
		...
		mcp.WithString("milestone", mcp.Description("If an `integer` is passed, it should refer to a milestone by its `number` field. If the string `*` is passed, issues with any milestone are accepted. If the string `none` is passed, issues without milestones are returned.")),
		mcp.WithString("state", mcp.Description("Indicates the state of the issues to return.")),
		mcp.WithString("assignee", mcp.Description("Can be the name of a user. Pass in `none` for issues with no assigned user, and `*` for issues assigned to any user.")),
		mcp.WithString("type", mcp.Description("Can be the name of an issue type. If the string `*` is passed, issues with any type are accepted. If the string `none` is passed, issues without type are returned.")),
		mcp.WithString("creator", mcp.Description("The user that created the issue.")),
		mcp.WithString("mentioned", mcp.Description("A user that's mentioned in the issue.")),
		mcp.WithString("labels", mcp.Description("A list of comma separated label names. Example: `bug,ui,@high`")),
		mcp.WithString("sort", mcp.Description("What to sort results by.")),
		mcp.WithString("direction", mcp.Description("The direction to sort the results by.")),
		mcp.WithString("since", mcp.Description("Only show results that were last updated after the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: `YYYY-MM-DDTHH:MM:SSZ`.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Issues_list_for_repoHandler(cfg),
	}
}
