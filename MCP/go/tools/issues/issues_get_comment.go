...
func Issues_get_commentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateIssues_get_commentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_issues_comments_comment_id",
		mcp.WithDescription("Get an issue comment"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Issues_get_commentHandler(cfg),
	}
}
