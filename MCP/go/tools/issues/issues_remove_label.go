...
func Issues_remove_labelHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/issues/%s/labels/%s%s", cfg.BaseURL, owner, repo, issue_number, name, queryString)
...
func CreateIssues_remove_labelTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_issues_issue_number_labels_name",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Issues_remove_labelHandler(cfg),
	}
}
