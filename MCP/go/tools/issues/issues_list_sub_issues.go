...
func Issues_list_sub_issuesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateIssues_list_sub_issuesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_issues_issue_number_sub_issues",
		mcp.WithDescription("List sub-issues"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Issues_list_sub_issuesHandler(cfg),
	}
}
