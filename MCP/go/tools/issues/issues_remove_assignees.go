...
func Issues_remove_assigneesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/issues/%s/assignees%s", cfg.BaseURL, owner, repo, issue_number, queryString)
...
func CreateIssues_remove_assigneesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_issues_issue_number_assignees",
		mcp.WithDescription("Remove assignees from an issue"),
		...
		mcp.WithArray("assignees", mcp.Description("Input parameter: Usernames of assignees to remove from an issue. _NOTE: Only users with push access can remove assignees from an issue. Assignees are silently ignored otherwise._")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Issues_remove_assigneesHandler(cfg),
	}
}
