...
func Issues_list_for_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/issues%s", cfg.BaseURL, org, queryString)
...
func CreateIssues_list_for_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_issues",
		mcp.WithDescription("List organization issues assigned to the authenticated user"),
		...
		mcp.WithString("type", mcp.Description("Can be the name of an issue type.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Issues_list_for_orgHandler(cfg),
	}
}
