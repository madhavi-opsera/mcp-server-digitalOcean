...
func Issues_list_milestonesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateIssues_list_milestonesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_milestones",
		...
		mcp.WithString("state", mcp.Description("The state of the milestone. Either `open`, `closed`, or `all`.")),
		mcp.WithString("sort", mcp.Description("What to sort results by. Either `due_on` or `completeness`.")),
		mcp.WithString("direction", mcp.Description("The direction of the sort. Either `asc` or `desc`.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Issues_list_milestonesHandler(cfg),
	}
}
