...
func Issues_unlockHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateIssues_unlockTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_issues_issue_number_lock",
		mcp.WithDescription("Unlock an issue"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Issues_unlockHandler(cfg),
	}
}
