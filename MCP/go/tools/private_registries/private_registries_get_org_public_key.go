...
func Private_registries_get_org_public_keyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/private-registries/public-key%s", cfg.BaseURL, org, queryString)
...
func CreatePrivate_registries_get_org_public_keyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_private-registries_public-key",
		mcp.WithDescription("Get private registries public key for an organization"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Private_registries_get_org_public_keyHandler(cfg),
	}
}
