...
func Git_get_commitHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/git/commits/%s%s", cfg.BaseURL, owner, repo, commit_sha, queryString)
...
func CreateGit_get_commitTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_git_commits_commit_sha",
		mcp.WithDescription("Get a commit object"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Git_get_commitHandler(cfg),
	}
}
