...
func Git_get_blobHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		file_shaVal, ok := args["file_sha"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: file_sha"), nil
		}
		file_sha, ok := file_shaVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: file_sha"), nil
		}
		...
		url := fmt.Sprintf("%s/repos/%s/%s/git/blobs/%s%s", cfg.BaseURL, owner, repo, file_sha, queryString)
...
func CreateGit_get_blobTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_git_blobs_file_sha",
		mcp.WithDescription("Get a blob"),
		...
		mcp.WithString("file_sha", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Git_get_blobHandler(cfg),
	}
}
