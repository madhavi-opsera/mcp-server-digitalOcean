...
func Git_get_refHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/git/ref/%s%s", cfg.BaseURL, owner, repo, ref, queryString)
...
func CreateGit_get_refTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_git_ref_ref",
		mcp.WithDescription("Get a reference"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Git_get_refHandler(cfg),
	}
}
