...
func Search_topicsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/search/topics%s", cfg.BaseURL, queryString)
...
func CreateSearch_topicsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_search_topics",
		mcp.WithDescription("Search topics"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Search_topicsHandler(cfg),
	}
}
