...
func Search_usersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/search/users%s", cfg.BaseURL, queryString)
...
func CreateSearch_usersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_search_users",
		...
		mcp.WithString("sort", mcp.Description("Sorts the results of your query by number of `followers` or `repositories`, or when the person `joined` GitHub. Default: [best match](https://docs.github.com/rest/search/search#ranking-search-results)")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Search_usersHandler(cfg),
	}
}
