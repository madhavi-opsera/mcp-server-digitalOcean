...
func Packages_list_docker_migration_conflicting_packages_for_organizationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/docker/conflicts%s", cfg.BaseURL, org, queryString)
...
func CreatePackages_list_docker_migration_conflicting_packages_for_organizationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_docker_conflicts",
		mcp.WithDescription("Get list of conflicting packages during Docker migration for organization"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Packages_list_docker_migration_conflicting_packages_for_organizationHandler(cfg),
	}
}
