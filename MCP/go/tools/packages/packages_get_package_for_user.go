...
func Packages_get_package_for_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePackages_get_package_for_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_username_packages_package_type_package_name",
		mcp.WithDescription("Get a package for a user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Packages_get_package_for_userHandler(cfg),
	}
}
