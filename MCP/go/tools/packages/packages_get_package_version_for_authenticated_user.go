...
func Packages_get_package_version_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePackages_get_package_version_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_user_packages_package_type_package_name_versions_package_version_id",
		mcp.WithDescription("Get a package version for the authenticated user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Packages_get_package_version_for_authenticated_userHandler(cfg),
	}
}
