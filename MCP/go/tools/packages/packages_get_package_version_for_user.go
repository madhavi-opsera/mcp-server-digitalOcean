...
func Packages_get_package_version_for_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/packages/%s/%s/versions/%s%s", cfg.BaseURL, package_type, package_name, package_version_id, username, queryString)
...
func CreatePackages_get_package_version_for_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_username_packages_package_type_package_name_versions_package_version_id",
		mcp.WithDescription("Get a package version for a user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Packages_get_package_version_for_userHandler(cfg),
	}
}
