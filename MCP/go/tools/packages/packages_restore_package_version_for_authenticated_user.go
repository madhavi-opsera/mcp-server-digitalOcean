...
func Packages_restore_package_version_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/packages/%s/%s/versions/%s/restore%s", cfg.BaseURL, package_type, package_name, package_version_id, queryString)
...
func CreatePackages_restore_package_version_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_user_packages_package_type_package_name_versions_package_version_id_restore",
		mcp.WithDescription("Restore a package version for the authenticated user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Packages_restore_package_version_for_authenticated_userHandler(cfg),
	}
}
