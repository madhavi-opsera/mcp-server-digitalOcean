...
func Packages_delete_package_for_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/packages/%s/%s%s", cfg.BaseURL, package_type, package_name, username, queryString)
...
func CreatePackages_delete_package_for_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_users_username_packages_package_type_package_name",
		mcp.WithDescription("Delete a package for a user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Packages_delete_package_for_userHandler(cfg),
	}
}
