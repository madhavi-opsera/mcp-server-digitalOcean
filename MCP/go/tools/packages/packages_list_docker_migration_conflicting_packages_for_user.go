...
func Packages_list_docker_migration_conflicting_packages_for_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/docker/conflicts%s", cfg.BaseURL, username, queryString)
...
func CreatePackages_list_docker_migration_conflicting_packages_for_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_username_docker_conflicts",
		mcp.WithDescription("Get list of conflicting packages during Docker migration for user"),
		mcp.WithString("username", mcp.Required(), mcp.Description("The handle for the GitHub user account.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Packages_list_docker_migration_conflicting_packages_for_userHandler(cfg),
	}
}
