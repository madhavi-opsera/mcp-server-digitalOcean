...
func Packages_delete_package_for_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePackages_delete_package_for_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_orgs_org_packages_package_type_package_name",
		mcp.WithDescription("Delete a package for an organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Packages_delete_package_for_orgHandler(cfg),
	}
}
