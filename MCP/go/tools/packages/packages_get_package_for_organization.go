...
func Packages_get_package_for_organizationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/packages/%s/%s%s", cfg.BaseURL, package_type, package_name, org, queryString)
...
func CreatePackages_get_package_for_organizationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_packages_package_type_package_name",
		mcp.WithDescription("Get a package for an organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Packages_get_package_for_organizationHandler(cfg),
	}
}
