...
func Packages_restore_package_version_for_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/packages/%s/%s/versions/%s/restore%s", cfg.BaseURL, package_type, package_name, org, package_version_id, queryString)
...
func CreatePackages_restore_package_version_for_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_orgs_org_packages_package_type_package_name_versions_package_version_id_restore",
		mcp.WithDescription("Restore package version for an organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Packages_restore_package_version_for_orgHandler(cfg),
	}
}
