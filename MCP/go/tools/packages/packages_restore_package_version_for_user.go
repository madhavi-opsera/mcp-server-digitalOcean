...
func Packages_restore_package_version_for_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/packages/%s/%s/versions/%s/restore%s", cfg.BaseURL, package_type, package_name, username, package_version_id, queryString)
...
func CreatePackages_restore_package_version_for_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_users_username_packages_package_type_package_name_versions_package_version_id_restore",
		mcp.WithDescription("Restore package version for a user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Packages_restore_package_version_for_userHandler(cfg),
	}
}
