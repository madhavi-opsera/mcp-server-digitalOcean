...
func Copilot_add_copilot_seats_for_teamsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/copilot/billing/selected_teams%s", cfg.BaseURL, org, queryString)
...
func CreateCopilot_add_copilot_seats_for_teamsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_orgs_org_copilot_billing_selected_teams",
		mcp.WithDescription("Add teams to the Copilot subscription for an organization"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
		mcp.WithArray("selected_teams", mcp.Required(), mcp.Description("Input parameter: List of team names within the organization to which to grant access to GitHub Copilot.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Copilot_add_copilot_seats_for_teamsHandler(cfg),
	}
}
