...
func Copilot_add_copilot_seats_for_usersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCopilot_add_copilot_seats_for_usersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_orgs_org_copilot_billing_selected_users",
		mcp.WithDescription("Add users to the Copilot subscription for an organization"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
		mcp.WithArray("selected_usernames", mcp.Required(), mcp.Description("Input parameter: The usernames of the organization members to be granted access to GitHub Copilot.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Copilot_add_copilot_seats_for_usersHandler(cfg),
	}
}
