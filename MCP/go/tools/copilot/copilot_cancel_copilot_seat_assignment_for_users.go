...
func Copilot_cancel_copilot_seat_assignment_for_usersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/copilot/billing/selected_users%s", cfg.BaseURL, org, queryString)
...
func CreateCopilot_cancel_copilot_seat_assignment_for_usersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_orgs_org_copilot_billing_selected_users",
		mcp.WithDescription("Remove users from the Copilot subscription for an organization"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
		mcp.WithArray("selected_usernames", mcp.Required(), mcp.Description("Input parameter: The usernames of the organization members for which to revoke access to GitHub Copilot.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Copilot_cancel_copilot_seat_assignment_for_usersHandler(cfg),
	}
}
