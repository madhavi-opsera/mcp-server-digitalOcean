...
func Codespaces_stop_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/codespaces/%s/stop%s", cfg.BaseURL, codespace_name, queryString)
...
func CreateCodespaces_stop_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_user_codespaces_codespace_name_stop",
		mcp.WithDescription("Stop a codespace for the authenticated user"),
		mcp.WithString("codespace_name", mcp.Required(), mcp.Description("The name of the codespace.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_stop_for_authenticated_userHandler(cfg),
	}
}
