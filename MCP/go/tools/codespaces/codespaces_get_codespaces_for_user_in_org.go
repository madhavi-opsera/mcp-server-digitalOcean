...
func Codespaces_get_codespaces_for_user_in_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/members/%s/codespaces%s", cfg.BaseURL, org, username, queryString)
...
func CreateCodespaces_get_codespaces_for_user_in_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_members_username_codespaces",
		mcp.WithDescription("List codespaces for a user in organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_get_codespaces_for_user_in_orgHandler(cfg),
	}
}
