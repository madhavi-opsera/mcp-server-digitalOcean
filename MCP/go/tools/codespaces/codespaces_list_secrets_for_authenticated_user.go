...
func Codespaces_list_secrets_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/codespaces/secrets%s", cfg.BaseURL, queryString)
...
func CreateCodespaces_list_secrets_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_user_codespaces_secrets",
		mcp.WithDescription("List secrets for the authenticated user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_list_secrets_for_authenticated_userHandler(cfg),
	}
}
