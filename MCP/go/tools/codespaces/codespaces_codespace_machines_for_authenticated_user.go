...
func Codespaces_codespace_machines_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/codespaces/%s/machines%s", cfg.BaseURL, codespace_name, queryString)
...
func CreateCodespaces_codespace_machines_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_user_codespaces_codespace_name_machines",
		mcp.WithDescription("List machine types for a codespace"),
		mcp.WithString("codespace_name", mcp.Required(), mcp.Description("The name of the codespace.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_codespace_machines_for_authenticated_userHandler(cfg),
	}
}
