...
func Codespaces_list_repo_secretsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/codespaces/secrets%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateCodespaces_list_repo_secretsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_codespaces_secrets",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_list_repo_secretsHandler(cfg),
	}
}
