...
func Codespaces_delete_codespaces_access_usersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/codespaces/access/selected_users%s", cfg.BaseURL, org, queryString)
...
func CreateCodespaces_delete_codespaces_access_usersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_orgs_org_codespaces_access_selected_users",
		mcp.WithDescription("Remove users from Codespaces access for an organization"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
		mcp.WithArray("selected_usernames", mcp.Required(), mcp.Description("Input parameter: The usernames of the organization members whose codespaces should not be billed to the organization.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_delete_codespaces_access_usersHandler(cfg),
	}
}
