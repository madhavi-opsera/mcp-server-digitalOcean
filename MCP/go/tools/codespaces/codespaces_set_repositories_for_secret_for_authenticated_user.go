...
func Codespaces_set_repositories_for_secret_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCodespaces_set_repositories_for_secret_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_user_codespaces_secrets_secret_name_repositories",
		mcp.WithDescription("Set selected repositories for a user secret"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_set_repositories_for_secret_for_authenticated_userHandler(cfg),
	}
}
