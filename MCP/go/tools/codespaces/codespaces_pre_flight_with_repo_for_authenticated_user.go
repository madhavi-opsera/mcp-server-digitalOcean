...
func Codespaces_pre_flight_with_repo_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/codespaces/new%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateCodespaces_pre_flight_with_repo_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_codespaces_new",
		mcp.WithDescription("Get default attributes for a codespace"),
		...
		mcp.WithString("ref", mcp.Description("The branch or commit to check for a default devcontainer path. If not specified, the default branch will be checked.")),
		mcp.WithString("client_ip", mcp.Description("An alternative IP for default location auto-detection, such as when proxying a request.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_pre_flight_with_repo_for_authenticated_userHandler(cfg),
	}
}
