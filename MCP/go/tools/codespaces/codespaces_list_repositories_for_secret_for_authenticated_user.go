...
func Codespaces_list_repositories_for_secret_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/codespaces/secrets/%s/repositories%s", cfg.BaseURL, secret_name, queryString)
...
func CreateCodespaces_list_repositories_for_secret_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_user_codespaces_secrets_secret_name_repositories",
		mcp.WithDescription("List selected repositories for a user secret"),
		mcp.WithString("secret_name", mcp.Required(), mcp.Description("The name of the secret.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_list_repositories_for_secret_for_authenticated_userHandler(cfg),
	}
}
