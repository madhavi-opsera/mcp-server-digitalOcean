...
func Codespaces_add_selected_repo_to_org_secretHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCodespaces_add_selected_repo_to_org_secretTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_orgs_org_codespaces_secrets_secret_name_repositories_repository_id",
		mcp.WithDescription("Add selected repository to an organization secret"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_add_selected_repo_to_org_secretHandler(cfg),
	}
}
