...
func Codespaces_set_codespaces_access_usersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCodespaces_set_codespaces_access_usersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_orgs_org_codespaces_access_selected_users",
		mcp.WithDescription("Add users to Codespaces access for an organization"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
		mcp.WithArray("selected_usernames", mcp.Required(), mcp.Description("Input parameter: The usernames of the organization members whose codespaces be billed to the organization.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_set_codespaces_access_usersHandler(cfg),
	}
}
