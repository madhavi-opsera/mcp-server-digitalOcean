...
func Codespaces_stop_in_organizationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/members/%s/codespaces/%s/stop%s", cfg.BaseURL, org, username, codespace_name, queryString)
...
func CreateCodespaces_stop_in_organizationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_orgs_org_members_username_codespaces_codespace_name_stop",
		mcp.WithDescription("Stop a codespace for an organization user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_stop_in_organizationHandler(cfg),
	}
}
