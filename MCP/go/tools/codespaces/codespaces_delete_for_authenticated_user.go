...
func Codespaces_delete_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCodespaces_delete_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_user_codespaces_codespace_name",
		mcp.WithDescription("Delete a codespace for the authenticated user"),
		mcp.WithString("codespace_name", mcp.Required(), mcp.Description("The name of the codespace.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_delete_for_authenticated_userHandler(cfg),
	}
}
