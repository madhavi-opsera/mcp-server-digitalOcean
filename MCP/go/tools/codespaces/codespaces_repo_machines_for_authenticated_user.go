...
func Codespaces_repo_machines_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["client_ip"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("client_ip=%v", val))
		}
		...
		url := fmt.Sprintf("%s/repos/%s/%s/codespaces/machines%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateCodespaces_repo_machines_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_codespaces_machines",
		mcp.WithDescription("List available machine types for a repository"),
		...
		mcp.WithString("location", mcp.Description("The location to check for available machines. Assigned by IP if not provided.")),
		mcp.WithString("client_ip", mcp.Description("IP for location auto-detection when proxying a request")),
		mcp.WithString("ref", mcp.Description("The branch or commit to check for prebuild availability and devcontainer restrictions.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Codespaces_repo_machines_for_authenticated_userHandler(cfg),
	}
}
