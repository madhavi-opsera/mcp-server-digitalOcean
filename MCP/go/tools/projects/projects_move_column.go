...
func Projects_move_columnHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/projects/columns/%s/moves%s", cfg.BaseURL, column_id, queryString)
...
func CreateProjects_move_columnTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_projects_columns_column_id_moves",
		mcp.WithDescription("Move a project column"),
		mcp.WithNumber("column_id", mcp.Required(), mcp.Description("The unique identifier of the column.")),
		mcp.WithString("position", mcp.Required(), mcp.Description("Input parameter: The position of the column in a project. Can be one of: `first`, `last`, or `after:<column_id>` to place after the specified column.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Projects_move_columnHandler(cfg),
	}
}
