...
func Activity_get_feedsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/feeds%s", cfg.BaseURL, queryString)
...
func CreateActivity_get_feedsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_feeds",
		mcp.WithDescription("Get feeds"),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Activity_get_feedsHandler(cfg),
	}
}
