...
func Activity_unstar_repo_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateActivity_unstar_repo_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_user_starred_owner_repo",
		mcp.WithDescription("Unstar a repository for the authenticated user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Activity_unstar_repo_for_authenticated_userHandler(cfg),
	}
}
