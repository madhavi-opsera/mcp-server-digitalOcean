...
func Activity_list_repo_eventsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/events%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateActivity_list_repo_eventsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_events",
		mcp.WithDescription("List repository events"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Activity_list_repo_eventsHandler(cfg),
	}
}
