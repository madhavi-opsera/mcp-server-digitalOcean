...
func Activity_delete_repo_subscriptionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateActivity_delete_repo_subscriptionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_subscription",
		mcp.WithDescription("Delete a repository subscription"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Activity_delete_repo_subscriptionHandler(cfg),
	}
}
