...
func Activity_check_repo_is_starred_by_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/starred/%s/%s%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateActivity_check_repo_is_starred_by_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_user_starred_owner_repo",
		mcp.WithDescription("Check if a repository is starred by the authenticated user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Activity_check_repo_is_starred_by_authenticated_userHandler(cfg),
	}
}
