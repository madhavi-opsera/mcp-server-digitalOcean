...
func Activity_list_public_events_for_repo_networkHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/networks/%s/%s/events%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateActivity_list_public_events_for_repo_networkTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_networks_owner_repo_events",
		mcp.WithDescription("List public events for a network of repositories"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Activity_list_public_events_for_repo_networkHandler(cfg),
	}
}
