...
func Migrations_set_lfs_preferenceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/import/lfs%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateMigrations_set_lfs_preferenceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_repos_owner_repo_import_lfs",
		mcp.WithDescription("Update Git LFS preference"),
		...
		mcp.WithString("use_lfs", mcp.Required(), mcp.Description("Input parameter: Whether to store large files during the import. `opt_in` means large files will be stored using Git LFS. `opt_out` means large files will be removed during the import.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Migrations_set_lfs_preferenceHandler(cfg),
	}
}
