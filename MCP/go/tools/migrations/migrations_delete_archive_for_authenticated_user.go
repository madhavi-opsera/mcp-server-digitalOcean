...
func Migrations_delete_archive_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/migrations/%s/archive%s", cfg.BaseURL, migration_id, queryString)
...
func CreateMigrations_delete_archive_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_user_migrations_migration_id_archive",
		mcp.WithDescription("Delete a user migration archive"),
		mcp.WithNumber("migration_id", mcp.Required(), mcp.Description("The unique identifier of the migration.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Migrations_delete_archive_for_authenticated_userHandler(cfg),
	}
}
