...
func Migrations_unlock_repo_for_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/migrations/%s/repos/%s/lock%s", cfg.BaseURL, org, migration_id, repo_name, queryString)
...
func CreateMigrations_unlock_repo_for_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_orgs_org_migrations_migration_id_repos_repo_name_lock",
		mcp.WithDescription("Unlock an organization repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Migrations_unlock_repo_for_orgHandler(cfg),
	}
}
