...
func Migrations_get_status_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/migrations/%s%s", cfg.BaseURL, migration_id, queryString)
...
func CreateMigrations_get_status_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_user_migrations_migration_id",
		mcp.WithDescription("Get a user migration status"),
		mcp.WithNumber("migration_id", mcp.Required(), mcp.Description("The unique identifier of the migration.")),
		mcp.WithArray("exclude", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Migrations_get_status_for_authenticated_userHandler(cfg),
	}
}
