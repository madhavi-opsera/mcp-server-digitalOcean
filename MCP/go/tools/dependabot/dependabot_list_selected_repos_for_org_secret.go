...
func Dependabot_list_selected_repos_for_org_secretHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/dependabot/secrets/%s/repositories%s", cfg.BaseURL, org, secret_name, queryString)
...
func CreateDependabot_list_selected_repos_for_org_secretTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_dependabot_secrets_secret_name_repositories",
		mcp.WithDescription("List selected repositories for an organization secret"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Dependabot_list_selected_repos_for_org_secretHandler(cfg),
	}
}
