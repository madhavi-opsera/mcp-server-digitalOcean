...
func Dependabot_remove_selected_repo_from_org_secretHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/dependabot/secrets/%s/repositories/%s%s", cfg.BaseURL, org, secret_name, repository_id, queryString)
...
func CreateDependabot_remove_selected_repo_from_org_secretTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_orgs_org_dependabot_secrets_secret_name_repositories_repository_id",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Dependabot_remove_selected_repo_from_org_secretHandler(cfg),
	}
}
