...
func Dependabot_set_repository_access_default_levelHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/organizations/%s/dependabot/repository-access/default-level%s", cfg.BaseURL, org, queryString)
...
func CreateDependabot_set_repository_access_default_levelTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_organizations_org_dependabot_repository-access_default-level",
		mcp.WithDescription("Set the default repository access level for Dependabot"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
		mcp.WithString("default_level", mcp.Required(), mcp.Description("Input parameter: The default repository access level for Dependabot updates.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Dependabot_set_repository_access_default_levelHandler(cfg),
	}
}
