...
func Dependabot_list_repo_secretsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/dependabot/secrets%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateDependabot_list_repo_secretsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_dependabot_secrets",
		mcp.WithDescription("List repository secrets"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Dependabot_list_repo_secretsHandler(cfg),
	}
}
