...
func Dependabot_delete_repo_secretHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/dependabot/secrets/%s%s", cfg.BaseURL, owner, repo, secret_name, queryString)
...
func CreateDependabot_delete_repo_secretTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_dependabot_secrets_secret_name",
		mcp.WithDescription("Delete a repository secret"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Dependabot_delete_repo_secretHandler(cfg),
	}
}
