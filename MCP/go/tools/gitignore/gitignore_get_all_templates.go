...
func Gitignore_get_all_templatesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/gitignore/templates%s", cfg.BaseURL, queryString)
...
func CreateGitignore_get_all_templatesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_gitignore_templates",
		mcp.WithDescription("Get all gitignore templates"),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Gitignore_get_all_templatesHandler(cfg),
	}
}
