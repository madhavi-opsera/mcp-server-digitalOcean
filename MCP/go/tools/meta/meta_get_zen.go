...
func Meta_get_zenHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/zen%s", cfg.BaseURL, queryString)
...
func CreateMeta_get_zenTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_zen",
		mcp.WithDescription("Get the Zen of GitHub"),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Meta_get_zenHandler(cfg),
	}
}
