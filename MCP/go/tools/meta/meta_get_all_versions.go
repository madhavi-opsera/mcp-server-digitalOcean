...
func Meta_get_all_versionsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/versions%s", cfg.BaseURL, queryString)
...
func CreateMeta_get_all_versionsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_versions",
		mcp.WithDescription("Get all API versions"),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Meta_get_all_versionsHandler(cfg),
	}
}
