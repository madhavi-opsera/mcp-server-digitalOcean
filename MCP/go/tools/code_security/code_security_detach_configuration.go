...
func Code_security_detach_configurationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/code-security/configurations/detach%s", cfg.BaseURL, org, queryString)
...
func CreateCode_security_detach_configurationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_orgs_org_code-security_configurations_detach",
		mcp.WithDescription("Detach configurations from repositories"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
		mcp.WithArray("selected_repository_ids", mcp.Description("Input parameter: An array of repository IDs to detach from configurations.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Code_security_detach_configurationHandler(cfg),
	}
}
