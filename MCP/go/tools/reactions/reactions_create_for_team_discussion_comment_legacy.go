...
func Reactions_create_for_team_discussion_comment_legacyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateReactions_create_for_team_discussion_comment_legacyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_teams_team_id_discussions_discussion_number_comments_comment_number_reactions",
		mcp.WithDescription("Create reaction for a team discussion comment (Legacy)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Reactions_create_for_team_discussion_comment_legacyHandler(cfg),
	}
}
