...
func Reactions_delete_for_team_discussionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/teams/%s/discussions/%s/reactions/%s%s", cfg.BaseURL, org, team_slug, discussion_number, reaction_id, queryString)
...
func CreateReactions_delete_for_team_discussionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_orgs_org_teams_team_slug_discussions_discussion_number_reactions_reaction_id",
		mcp.WithDescription("Delete team discussion reaction"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Reactions_delete_for_team_discussionHandler(cfg),
	}
}
