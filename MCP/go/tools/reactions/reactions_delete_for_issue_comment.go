...
func Reactions_delete_for_issue_commentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/issues/comments/%s/reactions/%s%s", cfg.BaseURL, owner, repo, comment_id, reaction_id, queryString)
...
func CreateReactions_delete_for_issue_commentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_issues_comments_comment_id_reactions_reaction_id",
		mcp.WithDescription("Delete an issue comment reaction"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Reactions_delete_for_issue_commentHandler(cfg),
	}
}
