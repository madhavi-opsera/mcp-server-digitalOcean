...
func Reactions_list_for_team_discussion_comment_in_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/teams/%s/discussions/%s/comments/%s/reactions%s", cfg.BaseURL, org, team_slug, discussion_number, comment_number, queryString)
...
func CreateReactions_list_for_team_discussion_comment_in_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_teams_team_slug_discussions_discussion_number_comments_comment_number_reactions",
		mcp.WithDescription("List reactions for a team discussion comment"),
		...
		mcp.WithString("content", mcp.Description("Returns a single [reaction type](https://docs.github.com/rest/reactions/reactions#about-reactions). Omit this parameter to list all reactions to a team discussion comment.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Reactions_list_for_team_discussion_comment_in_orgHandler(cfg),
	}
}
