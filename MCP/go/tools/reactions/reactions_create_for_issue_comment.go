...
func Reactions_create_for_issue_commentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateReactions_create_for_issue_commentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_issues_comments_comment_id_reactions",
		mcp.WithDescription("Create reaction for an issue comment"),
		...
		mcp.WithString("content", mcp.Required(), mcp.Description("Input parameter: The [reaction type](https://docs.github.com/rest/reactions/reactions#about-reactions) to add to the issue comment.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Reactions_create_for_issue_commentHandler(cfg),
	}
}
