...
func Reactions_create_for_releaseHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateReactions_create_for_releaseTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_releases_release_id_reactions",
		mcp.WithDescription("Create reaction for a release"),
		...
		mcp.WithString("content", mcp.Required(), mcp.Description("Input parameter: The [reaction type](https://docs.github.com/rest/reactions/reactions#about-reactions) to add to the release.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Reactions_create_for_releaseHandler(cfg),
	}
}
