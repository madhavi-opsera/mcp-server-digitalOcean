...
func Reactions_delete_for_releaseHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/releases/%s/reactions/%s%s", cfg.BaseURL, owner, repo, release_id, reaction_id, queryString)
...
func CreateReactions_delete_for_releaseTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_releases_release_id_reactions_reaction_id",
		mcp.WithDescription("Delete a release reaction"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Reactions_delete_for_releaseHandler(cfg),
	}
}
