...
func Reactions_delete_for_pull_request_commentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/pulls/comments/%s/reactions/%s%s", cfg.BaseURL, owner, repo, comment_id, reaction_id, queryString)
...
func CreateReactions_delete_for_pull_request_commentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_pulls_comments_comment_id_reactions_reaction_id",
		mcp.WithDescription("Delete a pull request comment reaction"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Reactions_delete_for_pull_request_commentHandler(cfg),
	}
}
