...
func Reactions_list_for_pull_request_review_commentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/pulls/comments/%s/reactions%s", cfg.BaseURL, owner, repo, comment_id, queryString)
...
func CreateReactions_list_for_pull_request_review_commentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_pulls_comments_comment_id_reactions",
		mcp.WithDescription("List reactions for a pull request review comment"),
		...
		mcp.WithString("content", mcp.Description("Returns a single [reaction type](https://docs.github.com/rest/reactions/reactions#about-reactions). Omit this parameter to list all reactions to a pull request review comment.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Reactions_list_for_pull_request_review_commentHandler(cfg),
	}
}
