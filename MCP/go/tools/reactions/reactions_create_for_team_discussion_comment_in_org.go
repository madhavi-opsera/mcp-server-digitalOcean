...
func Reactions_create_for_team_discussion_comment_in_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateReactions_create_for_team_discussion_comment_in_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_orgs_org_teams_team_slug_discussions_discussion_number_comments_comment_number_reactions",
		mcp.WithDescription("Create reaction for a team discussion comment"),
		...
		mcp.WithString("content", mcp.Required(), mcp.Description("Input parameter: The [reaction type](https://docs.github.com/rest/reactions/reactions#about-reactions) to add to the team discussion comment.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Reactions_create_for_team_discussion_comment_in_orgHandler(cfg),
	}
}
