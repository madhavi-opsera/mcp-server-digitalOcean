...
func Pulls_update_branchHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/pulls/%s/update-branch%s", cfg.BaseURL, owner, repo, pull_number, queryString)
...
func CreatePulls_update_branchTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_repos_owner_repo_pulls_pull_number_update-branch",
		mcp.WithDescription("Update a pull request branch"),
