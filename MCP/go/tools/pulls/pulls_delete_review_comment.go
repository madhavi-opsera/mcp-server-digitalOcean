...
func Pulls_delete_review_commentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePulls_delete_review_commentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_pulls_comments_comment_id",
		mcp.WithDescription("Delete a review comment for a pull request"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Pulls_delete_review_commentHandler(cfg),
	}
}
