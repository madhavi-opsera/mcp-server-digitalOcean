...
func Pulls_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["head"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("head=%v", val))
		}
		if val, ok := args["base"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("base=%v", val))
		}
...
func CreatePulls_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_pulls",
		...
		mcp.WithString("state", mcp.Description("Either `open`, `closed`, or `all` to filter by state.")),
		mcp.WithString("head", mcp.Description("Filter pulls by head user or head organization and branch name in the format of `user:ref-name` or `organization:ref-name`. For example: `github:new-script-format` or `octocat:test-branch`.")),
		mcp.WithString("base", mcp.Description("Filter pulls by base branch name. Example: `gh-pages`.")),
		mcp.WithString("sort", mcp.Description("What to sort results by. `popularity` will sort by the number of comments. `long-running` will sort by date created and will limit the results to pull requests that have been open for more than a month and have had activity within the past month.")),
		mcp.WithString("direction", mcp.Description("The direction of the sort. Default: `desc` when sort is `created` or sort is not specified, otherwise `asc`.")),
