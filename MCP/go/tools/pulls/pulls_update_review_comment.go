...
func Pulls_update_review_commentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/pulls/comments/%s%s", cfg.BaseURL, owner, repo, comment_id, queryString)
...
func CreatePulls_update_review_commentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_repos_owner_repo_pulls_comments_comment_id",
		mcp.WithDescription("Update a review comment for a pull request"),
		...
		mcp.WithString("body", mcp.Required(), mcp.Description("Input parameter: The text of the reply to the review comment.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Pulls_update_review_commentHandler(cfg),
	}
}
