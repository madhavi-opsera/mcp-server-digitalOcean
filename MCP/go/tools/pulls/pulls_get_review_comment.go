...
func Pulls_get_review_commentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePulls_get_review_commentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_pulls_comments_comment_id",
		mcp.WithDescription("Get a review comment for a pull request"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Pulls_get_review_commentHandler(cfg),
	}
}
