...
func Pulls_create_reply_for_review_commentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/pulls/%s/comments/%s/replies%s", cfg.BaseURL, owner, repo, pull_number, comment_id, queryString)
...
func CreatePulls_create_reply_for_review_commentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_pulls_pull_number_comments_comment_id_replies",
		mcp.WithDescription("Create a reply for a review comment"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Pulls_create_reply_for_review_commentHandler(cfg),
	}
}
