...
func Pulls_update_reviewHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePulls_update_reviewTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_repos_owner_repo_pulls_pull_number_reviews_review_id",
		mcp.WithDescription("Update a review for a pull request"),
		...
		mcp.WithString("body", mcp.Required(), mcp.Description("Input parameter: The body text of the pull request review.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Pulls_update_reviewHandler(cfg),
	}
}
