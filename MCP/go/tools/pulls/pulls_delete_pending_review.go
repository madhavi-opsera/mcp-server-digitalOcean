...
func Pulls_delete_pending_reviewHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/pulls/%s/reviews/%s%s", cfg.BaseURL, owner, repo, pull_number, review_id, queryString)
...
func CreatePulls_delete_pending_reviewTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_pulls_pull_number_reviews_review_id",
		mcp.WithDescription("Delete a pending review for a pull request"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Pulls_delete_pending_reviewHandler(cfg),
	}
}
