...
func Actions_get_job_for_workflow_runHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/jobs/%s%s", cfg.BaseURL, owner, repo, job_id, queryString)
...
func CreateActions_get_job_for_workflow_runTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_actions_jobs_job_id",
		mcp.WithDescription("Get a job for a workflow run"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_get_job_for_workflow_runHandler(cfg),
	}
}
