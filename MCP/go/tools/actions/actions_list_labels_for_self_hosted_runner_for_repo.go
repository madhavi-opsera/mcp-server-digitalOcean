...
func Actions_list_labels_for_self_hosted_runner_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateActions_list_labels_for_self_hosted_runner_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_actions_runners_runner_id_labels",
		mcp.WithDescription("List labels for a self-hosted runner for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_list_labels_for_self_hosted_runner_for_repoHandler(cfg),
	}
}
