...
func Actions_set_custom_labels_for_self_hosted_runner_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/runners/%s/labels%s", cfg.BaseURL, owner, repo, runner_id, queryString)
...
func CreateActions_set_custom_labels_for_self_hosted_runner_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_repos_owner_repo_actions_runners_runner_id_labels",
		mcp.WithDescription("Set custom labels for a self-hosted runner for a repository"),
		...
		mcp.WithArray("labels", mcp.Required(), mcp.Description("Input parameter: The names of the custom labels to set for the runner. You can pass an empty array to remove all custom labels.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Actions_set_custom_labels_for_self_hosted_runner_for_repoHandler(cfg),
	}
}
