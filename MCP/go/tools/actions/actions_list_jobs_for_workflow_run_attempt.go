...
func Actions_list_jobs_for_workflow_run_attemptHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/runs/%s/attempts/%s/jobs%s", cfg.BaseURL, owner, repo, run_id, attempt_number, queryString)
...
func CreateActions_list_jobs_for_workflow_run_attemptTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_actions_runs_run_id_attempts_attempt_number_jobs",
		mcp.WithDescription("List jobs for a workflow run attempt"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_list_jobs_for_workflow_run_attemptHandler(cfg),
	}
}
