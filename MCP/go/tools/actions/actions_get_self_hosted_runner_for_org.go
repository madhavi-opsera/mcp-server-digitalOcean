...
func Actions_get_self_hosted_runner_for_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateActions_get_self_hosted_runner_for_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_actions_runners_runner_id",
		mcp.WithDescription("Get a self-hosted runner for an organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_get_self_hosted_runner_for_orgHandler(cfg),
	}
}
