...
func Actions_delete_environment_secretHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateActions_delete_environment_secretTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_environments_environment_name_secrets_secret_name",
		mcp.WithDescription("Delete an environment secret"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_delete_environment_secretHandler(cfg),
	}
}
