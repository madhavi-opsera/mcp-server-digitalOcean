...
func Actions_re_run_job_for_workflow_runHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		job_idVal, ok := args["job_id"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: job_id"), nil
		}
		job_id, ok := job_idVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: job_id"), nil
		}
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/jobs/%s/rerun%s", cfg.BaseURL, owner, repo, job_id, queryString)
...
func CreateActions_re_run_job_for_workflow_runTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_actions_jobs_job_id_rerun",
		mcp.WithDescription("Re-run a job from a workflow run"),
		...
		mcp.WithNumber("job_id", mcp.Required(), mcp.Description("The unique identifier of the job.")),
		mcp.WithBoolean("enable_debug_logging", mcp.Description("Input parameter: Whether to enable debug logging for the re-run.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Actions_re_run_job_for_workflow_runHandler(cfg),
	}
}
