...
func Actions_disable_workflowHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/workflows/%s/disable%s", cfg.BaseURL, owner, repo, workflow_id, queryString)
...
func CreateActions_disable_workflowTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_repos_owner_repo_actions_workflows_workflow_id_disable",
		mcp.WithDescription("Disable a workflow"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_disable_workflowHandler(cfg),
	}
}
