...
func Actions_remove_all_custom_labels_from_self_hosted_runner_for_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateActions_remove_all_custom_labels_from_self_hosted_runner_for_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_orgs_org_actions_runners_runner_id_labels",
		mcp.WithDescription("Remove all custom labels from a self-hosted runner for an organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_remove_all_custom_labels_from_self_hosted_runner_for_orgHandler(cfg),
	}
}
