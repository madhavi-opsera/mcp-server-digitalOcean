...
func Actions_delete_workflow_run_logsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateActions_delete_workflow_run_logsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_actions_runs_run_id_logs",
		mcp.WithDescription("Delete workflow run logs"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_delete_workflow_run_logsHandler(cfg),
	}
}
