...
func Actions_set_selected_repositories_enabled_github_actions_organizationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/actions/permissions/repositories%s", cfg.BaseURL, org, queryString)
...
func CreateActions_set_selected_repositories_enabled_github_actions_organizationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_orgs_org_actions_permissions_repositories",
		mcp.WithDescription("Set selected repositories enabled for GitHub Actions in an organization"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
		mcp.WithArray("selected_repository_ids", mcp.Required(), mcp.Description("Input parameter: List of repository IDs to enable for GitHub Actions.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Actions_set_selected_repositories_enabled_github_actions_organizationHandler(cfg),
	}
}
