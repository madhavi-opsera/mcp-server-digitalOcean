...
func Actions_get_github_actions_default_workflow_permissions_organizationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/actions/permissions/workflow%s", cfg.BaseURL, org, queryString)
...
func CreateActions_get_github_actions_default_workflow_permissions_organizationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_actions_permissions_workflow",
		mcp.WithDescription("Get default workflow permissions for an organization"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Actions_get_github_actions_default_workflow_permissions_organizationHandler(cfg),
	}
}
