...
func Actions_delete_actions_cache_by_idHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		cache_idVal, ok := args["cache_id"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: cache_id"), nil
		}
		cache_id, ok := cache_idVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: cache_id"), nil
		}
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/caches/%s%s", cfg.BaseURL, owner, repo, cache_id, queryString)
...
func CreateActions_delete_actions_cache_by_idTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_actions_caches_cache_id",
		mcp.WithDescription("Delete a GitHub Actions cache for a repository (using a cache ID)"),
		...
		mcp.WithNumber("cache_id", mcp.Required(), mcp.Description("The unique identifier of the GitHub Actions cache.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Actions_delete_actions_cache_by_idHandler(cfg),
	}
}
