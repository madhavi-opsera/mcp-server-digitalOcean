...
func Actions_list_artifacts_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/artifacts%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateActions_list_artifacts_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_actions_artifacts",
		mcp.WithDescription("List artifacts for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_list_artifacts_for_repoHandler(cfg),
	}
}
