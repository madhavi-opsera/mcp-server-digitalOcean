...
func Actions_add_self_hosted_runner_to_group_for_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateActions_add_self_hosted_runner_to_group_for_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_orgs_org_actions_runner-groups_runner_group_id_runners_runner_id",
		mcp.WithDescription("Add a self-hosted runner to a group for an organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_add_self_hosted_runner_to_group_for_orgHandler(cfg),
	}
}
