...
func Actions_get_self_hosted_runner_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateActions_get_self_hosted_runner_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_actions_runners_runner_id",
		mcp.WithDescription("Get a self-hosted runner for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_get_self_hosted_runner_for_repoHandler(cfg),
	}
}
