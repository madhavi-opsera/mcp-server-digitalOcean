...
func Actions_delete_self_hosted_runner_from_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/actions/runners/%s%s", cfg.BaseURL, org, runner_id, queryString)
...
func CreateActions_delete_self_hosted_runner_from_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_orgs_org_actions_runners_runner_id",
		mcp.WithDescription("Delete a self-hosted runner from an organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_delete_self_hosted_runner_from_orgHandler(cfg),
	}
}
