...
func Actions_list_self_hosted_runner_groups_for_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["visible_to_repository"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("visible_to_repository=%v", val))
		}
...
func CreateActions_list_self_hosted_runner_groups_for_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_actions_runner-groups",
		mcp.WithDescription("List self-hosted runner groups for an organization"),
		...
		mcp.WithString("visible_to_repository", mcp.Description("Only return runner groups that are allowed to be used by this repository.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Actions_list_self_hosted_runner_groups_for_orgHandler(cfg),
	}
}
