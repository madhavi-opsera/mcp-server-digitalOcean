...
func Actions_list_repo_organization_variablesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/organization-variables%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateActions_list_repo_organization_variablesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_actions_organization-variables",
		mcp.WithDescription("List repository organization variables"),
		...
		mcp.WithNumber("per_page", mcp.Description("The number of results per page (max 30). For more information, see \"[Using pagination in the REST API](https://docs.github.com/rest/using-the-rest-api/using-pagination-in-the-rest-api).\"")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_list_repo_organization_variablesHandler(cfg),
	}
}
