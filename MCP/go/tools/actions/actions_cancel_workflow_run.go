...
func Actions_cancel_workflow_runHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/runs/%s/cancel%s", cfg.BaseURL, owner, repo, run_id, queryString)
...
func CreateActions_cancel_workflow_runTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_actions_runs_run_id_cancel",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_cancel_workflow_runHandler(cfg),
	}
}
