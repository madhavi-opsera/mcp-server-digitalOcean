...
func Actions_add_custom_labels_to_self_hosted_runner_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateActions_add_custom_labels_to_self_hosted_runner_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_actions_runners_runner_id_labels",
		mcp.WithDescription("Add custom labels to a self-hosted runner for a repository"),
		...
		mcp.WithArray("labels", mcp.Required(), mcp.Description("Input parameter: The names of the custom labels to add to the runner.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Actions_add_custom_labels_to_self_hosted_runner_for_repoHandler(cfg),
	}
}
