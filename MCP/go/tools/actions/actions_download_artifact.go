...
func Actions_download_artifactHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		artifact_idVal, ok := args["artifact_id"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: artifact_id"), nil
		}
		artifact_id, ok := artifact_idVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: artifact_id"), nil
		}
		archive_formatVal, ok := args["archive_format"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: archive_format"), nil
		}
		archive_format, ok := archive_formatVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: archive_format"), nil
		}
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/artifacts/%s/%s%s", cfg.BaseURL, owner, repo, artifact_id, archive_format, queryString)
...
func CreateActions_download_artifactTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_actions_artifacts_artifact_id_archive_format",
		mcp.WithDescription("Download an artifact"),
		...
		mcp.WithNumber("artifact_id", mcp.Required(), mcp.Description("The unique identifier of the artifact.")),
		mcp.WithString("archive_format", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Actions_download_artifactHandler(cfg),
	}
}
