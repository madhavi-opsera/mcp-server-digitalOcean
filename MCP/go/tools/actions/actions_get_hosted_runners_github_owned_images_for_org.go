...
func Actions_get_hosted_runners_github_owned_images_for_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/actions/hosted-runners/images/github-owned%s", cfg.BaseURL, org, queryString)
...
func CreateActions_get_hosted_runners_github_owned_images_for_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_actions_hosted-runners_images_github-owned",
		mcp.WithDescription("Get GitHub-owned images for GitHub-hosted runners in an organization"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Actions_get_hosted_runners_github_owned_images_for_orgHandler(cfg),
	}
}
