...
func Actions_remove_all_custom_labels_from_self_hosted_runner_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateActions_remove_all_custom_labels_from_self_hosted_runner_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_actions_runners_runner_id_labels",
		mcp.WithDescription("Remove all custom labels from a self-hosted runner for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_remove_all_custom_labels_from_self_hosted_runner_for_repoHandler(cfg),
	}
}
