...
func Actions_set_repo_access_to_self_hosted_runner_group_in_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/actions/runner-groups/%s/repositories%s", cfg.BaseURL, org, runner_group_id, queryString)
...
func CreateActions_set_repo_access_to_self_hosted_runner_group_in_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_orgs_org_actions_runner-groups_runner_group_id_repositories",
		mcp.WithDescription("Set repository access for a self-hosted runner group in an organization"),
		...
		mcp.WithArray("selected_repository_ids", mcp.Required(), mcp.Description("Input parameter: List of repository IDs that can access the runner group.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Actions_set_repo_access_to_self_hosted_runner_group_in_orgHandler(cfg),
	}
}
