...
func Actions_remove_custom_label_from_self_hosted_runner_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/runners/%s/labels/%s%s", cfg.BaseURL, owner, repo, runner_id, name, queryString)
...
func CreateActions_remove_custom_label_from_self_hosted_runner_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_actions_runners_runner_id_labels_name",
		mcp.WithDescription("Remove a custom label from a self-hosted runner for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_remove_custom_label_from_self_hosted_runner_for_repoHandler(cfg),
	}
}
