...
func Actions_add_custom_labels_to_self_hosted_runner_for_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateActions_add_custom_labels_to_self_hosted_runner_for_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_orgs_org_actions_runners_runner_id_labels",
		mcp.WithDescription("Add custom labels to a self-hosted runner for an organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_add_custom_labels_to_self_hosted_runner_for_orgHandler(cfg),
	}
}
