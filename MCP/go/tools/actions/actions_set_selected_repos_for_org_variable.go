...
func Actions_set_selected_repos_for_org_variableHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/actions/variables/%s/repositories%s", cfg.BaseURL, org, name, queryString)
...
func CreateActions_set_selected_repos_for_org_variableTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_orgs_org_actions_variables_name_repositories",
		mcp.WithDescription("Set selected repositories for an organization variable"),
		...
		mcp.WithArray("selected_repository_ids", mcp.Required(), mcp.Description("Input parameter: The IDs of the repositories that can access the organization variable.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Actions_set_selected_repos_for_org_variableHandler(cfg),
	}
}
