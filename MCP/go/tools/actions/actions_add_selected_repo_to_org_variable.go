...
func Actions_add_selected_repo_to_org_variableHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/actions/variables/%s/repositories/%s%s", cfg.BaseURL, org, name, repository_id, queryString)
...
func CreateActions_add_selected_repo_to_org_variableTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_orgs_org_actions_variables_name_repositories_repository_id",
		mcp.WithDescription("Add selected repository to an organization variable"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_add_selected_repo_to_org_variableHandler(cfg),
	}
}
