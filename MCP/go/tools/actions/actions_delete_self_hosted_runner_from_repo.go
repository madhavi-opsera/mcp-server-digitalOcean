...
func Actions_delete_self_hosted_runner_from_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/runners/%s%s", cfg.BaseURL, owner, repo, runner_id, queryString)
...
func CreateActions_delete_self_hosted_runner_from_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_actions_runners_runner_id",
		mcp.WithDescription("Delete a self-hosted runner from a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_delete_self_hosted_runner_from_repoHandler(cfg),
	}
}
