...
func Actions_re_run_workflow_failed_jobsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/runs/%s/rerun-failed-jobs%s", cfg.BaseURL, owner, repo, run_id, queryString)
...
func CreateActions_re_run_workflow_failed_jobsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_actions_runs_run_id_rerun-failed-jobs",
		mcp.WithDescription("Re-run failed jobs from a workflow run"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_re_run_workflow_failed_jobsHandler(cfg),
	}
}
