...
func Actions_enable_workflowHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/workflows/%s/enable%s", cfg.BaseURL, owner, repo, workflow_id, queryString)
...
func CreateActions_enable_workflowTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_repos_owner_repo_actions_workflows_workflow_id_enable",
		mcp.WithDescription("Enable a workflow"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_enable_workflowHandler(cfg),
	}
}
