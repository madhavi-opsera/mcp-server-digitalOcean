...
func Actions_enable_selected_repository_github_actions_organizationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/actions/permissions/repositories/%s%s", cfg.BaseURL, org, repository_id, queryString)
...
func CreateActions_enable_selected_repository_github_actions_organizationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_orgs_org_actions_permissions_repositories_repository_id",
		mcp.WithDescription("Enable a selected repository for GitHub Actions in an organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_enable_selected_repository_github_actions_organizationHandler(cfg),
	}
}
