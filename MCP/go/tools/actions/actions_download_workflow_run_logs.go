...
func Actions_download_workflow_run_logsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/runs/%s/logs%s", cfg.BaseURL, owner, repo, run_id, queryString)
...
func CreateActions_download_workflow_run_logsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_actions_runs_run_id_logs",
		mcp.WithDescription("Download workflow run logs"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_download_workflow_run_logsHandler(cfg),
	}
}
