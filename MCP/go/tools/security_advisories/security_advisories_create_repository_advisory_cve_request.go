...
func Security_advisories_create_repository_advisory_cve_requestHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/security-advisories/%s/cve%s", cfg.BaseURL, owner, repo, ghsa_id, queryString)
...
func CreateSecurity_advisories_create_repository_advisory_cve_requestTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_security-advisories_ghsa_id_cve",
		mcp.WithDescription("Request a CVE for a repository security advisory"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Security_advisories_create_repository_advisory_cve_requestHandler(cfg),
	}
}
