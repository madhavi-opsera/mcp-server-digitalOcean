...
func Security_advisories_list_repository_advisoriesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateSecurity_advisories_list_repository_advisoriesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_security-advisories",
		mcp.WithDescription("List repository security advisories"),
		...
		mcp.WithString("state", mcp.Description("Filter by state of the repository advisories. Only advisories of this state will be returned.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Security_advisories_list_repository_advisoriesHandler(cfg),
	}
}
