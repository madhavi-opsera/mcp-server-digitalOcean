...
func Security_advisories_list_org_repository_advisoriesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/security-advisories%s", cfg.BaseURL, org, queryString)
...
func CreateSecurity_advisories_list_org_repository_advisoriesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_security-advisories",
		mcp.WithDescription("List repository security advisories for an organization"),
		...
		mcp.WithNumber("per_page", mcp.Description("The number of advisories to return per page. For more information, see \"[Using pagination in the REST API](https://docs.github.com/rest/using-the-rest-api/using-pagination-in-the-rest-api).\"")),
		mcp.WithString("state", mcp.Description("Filter by the state of the repository advisories. Only advisories of this state will be returned.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Security_advisories_list_org_repository_advisoriesHandler(cfg),
	}
}
