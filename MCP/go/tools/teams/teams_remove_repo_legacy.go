...
func Teams_remove_repo_legacyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateTeams_remove_repo_legacyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_teams_team_id_repos_owner_repo",
		mcp.WithDescription("Remove a repository from a team (Legacy)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_remove_repo_legacyHandler(cfg),
	}
}
