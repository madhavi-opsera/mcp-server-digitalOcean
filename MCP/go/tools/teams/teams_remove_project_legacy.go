...
func Teams_remove_project_legacyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateTeams_remove_project_legacyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_teams_team_id_projects_project_id",
		mcp.WithDescription("Remove a project from a team (Legacy)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_remove_project_legacyHandler(cfg),
	}
}
