...
func Teams_remove_member_legacyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/teams/%s/members/%s%s", cfg.BaseURL, team_id, username, queryString)
...
func CreateTeams_remove_member_legacyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_teams_team_id_members_username",
		mcp.WithDescription("Remove team member (Legacy)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_remove_member_legacyHandler(cfg),
	}
}
