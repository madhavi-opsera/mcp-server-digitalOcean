...
func Teams_check_permissions_for_project_in_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/teams/%s/projects/%s%s", cfg.BaseURL, org, team_slug, project_id, queryString)
...
func CreateTeams_check_permissions_for_project_in_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_teams_team_slug_projects_project_id",
		mcp.WithDescription("Check team permissions for a project"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_check_permissions_for_project_in_orgHandler(cfg),
	}
}
