...
func Teams_delete_discussion_comment_legacyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/teams/%s/discussions/%s/comments/%s%s", cfg.BaseURL, team_id, discussion_number, comment_number, queryString)
...
func CreateTeams_delete_discussion_comment_legacyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_teams_team_id_discussions_discussion_number_comments_comment_number",
		mcp.WithDescription("Delete a discussion comment (Legacy)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_delete_discussion_comment_legacyHandler(cfg),
	}
}
