...
func Teams_update_discussion_comment_legacyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateTeams_update_discussion_comment_legacyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_teams_team_id_discussions_discussion_number_comments_comment_number",
		mcp.WithDescription("Update a discussion comment (Legacy)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_update_discussion_comment_legacyHandler(cfg),
	}
}
