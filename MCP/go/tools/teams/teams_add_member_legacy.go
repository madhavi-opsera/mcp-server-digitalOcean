...
func Teams_add_member_legacyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateTeams_add_member_legacyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_teams_team_id_members_username",
		mcp.WithDescription("Add team member (Legacy)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_add_member_legacyHandler(cfg),
	}
}
