...
func Teams_remove_membership_for_user_legacyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/teams/%s/memberships/%s%s", cfg.BaseURL, team_id, username, queryString)
...
func CreateTeams_remove_membership_for_user_legacyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_teams_team_id_memberships_username",
		mcp.WithDescription("Remove team membership for a user (Legacy)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_remove_membership_for_user_legacyHandler(cfg),
	}
}
