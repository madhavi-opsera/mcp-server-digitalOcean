...
func Teams_list_discussions_in_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateTeams_list_discussions_in_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_teams_team_slug_discussions",
		mcp.WithDescription("List discussions"),
		...
		mcp.WithString("pinned", mcp.Description("Pinned discussions only filter")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Teams_list_discussions_in_orgHandler(cfg),
	}
}
