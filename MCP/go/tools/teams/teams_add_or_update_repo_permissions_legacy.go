...
func Teams_add_or_update_repo_permissions_legacyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/teams/%s/repos/%s/%s%s", cfg.BaseURL, team_id, owner, repo, queryString)
...
func CreateTeams_add_or_update_repo_permissions_legacyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_teams_team_id_repos_owner_repo",
		mcp.WithDescription("Add or update team repository permissions (Legacy)"),
		...
		mcp.WithString("permission", mcp.Description("Input parameter: The permission to grant the team on this repository. If no permission is specified, the team's `permission` attribute will be used to determine what permission to grant the team on this repository.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Teams_add_or_update_repo_permissions_legacyHandler(cfg),
	}
}
