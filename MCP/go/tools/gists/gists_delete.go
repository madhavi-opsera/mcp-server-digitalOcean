...
func Gists_deleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/gists/%s%s", cfg.BaseURL, gist_id, queryString)
...
func CreateGists_deleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_gists_gist_id",
		mcp.WithDescription("Delete a gist"),
		mcp.WithString("gist_id", mcp.Required(), mcp.Description("The unique identifier of the gist.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Gists_deleteHandler(cfg),
	}
}
