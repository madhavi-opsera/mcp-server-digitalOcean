...
func Gists_get_revisionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/gists/%s/%s%s", cfg.BaseURL, gist_id, sha, queryString)
...
func CreateGists_get_revisionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_gists_gist_id_sha",
		mcp.WithDescription("Get a gist revision"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Gists_get_revisionHandler(cfg),
	}
}
