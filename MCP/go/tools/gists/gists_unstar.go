...
func Gists_unstarHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGists_unstarTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_gists_gist_id_star",
		mcp.WithDescription("Unstar a gist"),
		mcp.WithString("gist_id", mcp.Required(), mcp.Description("The unique identifier of the gist.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Gists_unstarHandler(cfg),
	}
}
