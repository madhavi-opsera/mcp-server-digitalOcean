...
func Gists_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGists_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_gists_gist_id",
		mcp.WithDescription("Get a gist"),
		mcp.WithString("gist_id", mcp.Required(), mcp.Description("The unique identifier of the gist.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Gists_getHandler(cfg),
	}
}
