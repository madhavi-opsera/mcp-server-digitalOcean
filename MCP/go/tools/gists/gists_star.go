...
func Gists_starHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGists_starTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_gists_gist_id_star",
		mcp.WithDescription("Star a gist"),
		mcp.WithString("gist_id", mcp.Required(), mcp.Description("The unique identifier of the gist.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Gists_starHandler(cfg),
	}
}
