...
func Gists_check_is_starredHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/gists/%s/star%s", cfg.BaseURL, gist_id, queryString)
...
func CreateGists_check_is_starredTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_gists_gist_id_star",
		mcp.WithDescription("Check if a gist is starred"),
		mcp.WithString("gist_id", mcp.Required(), mcp.Description("The unique identifier of the gist.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Gists_check_is_starredHandler(cfg),
	}
}
