...
func Users_check_blockedHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/blocks/%s%s", cfg.BaseURL, username, queryString)
...
func CreateUsers_check_blockedTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_user_blocks_username",
		mcp.WithDescription("Check if a user is blocked by the authenticated user"),
		mcp.WithString("username", mcp.Required(), mcp.Description("The handle for the GitHub user account.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Users_check_blockedHandler(cfg),
	}
}
