...
func Users_check_person_is_followed_by_authenticatedHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/following/%s%s", cfg.BaseURL, username, queryString)
...
func CreateUsers_check_person_is_followed_by_authenticatedTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_user_following_username",
		mcp.WithDescription("Check if a person is followed by the authenticated user"),
		mcp.WithString("username", mcp.Required(), mcp.Description("The handle for the GitHub user account.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Users_check_person_is_followed_by_authenticatedHandler(cfg),
	}
}
