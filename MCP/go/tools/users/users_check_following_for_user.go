...
func Users_check_following_for_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		target_userVal, ok := args["target_user"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: target_user"), nil
		}
		target_user, ok := target_userVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: target_user"), nil
		}
		...
		url := fmt.Sprintf("%s/users/%s/following/%s%s", cfg.BaseURL, username, target_user, queryString)
...
func CreateUsers_check_following_for_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_users_username_following_target_user",
		mcp.WithDescription("Check if a user follows another user"),
		mcp.WithString("username", mcp.Required(), mcp.Description("The handle for the GitHub user account.")),
		mcp.WithString("target_user", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Users_check_following_for_userHandler(cfg),
	}
}
