...
func Users_get_public_ssh_key_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateUsers_get_public_ssh_key_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_user_keys_key_id",
		mcp.WithDescription("Get a public SSH key for the authenticated user"),
		mcp.WithNumber("key_id", mcp.Required(), mcp.Description("The unique identifier of the key.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Users_get_public_ssh_key_for_authenticated_userHandler(cfg),
	}
}
