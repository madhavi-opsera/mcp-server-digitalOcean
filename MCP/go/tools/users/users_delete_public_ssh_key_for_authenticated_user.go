...
func Users_delete_public_ssh_key_for_authenticated_userHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/user/keys/%s%s", cfg.BaseURL, key_id, queryString)
...
func CreateUsers_delete_public_ssh_key_for_authenticated_userTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_user_keys_key_id",
		mcp.WithDescription("Delete a public SSH key for the authenticated user"),
		mcp.WithNumber("key_id", mcp.Required(), mcp.Description("The unique identifier of the key.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Users_delete_public_ssh_key_for_authenticated_userHandler(cfg),
	}
}
